...
say(){ printf '\033[1;36m[CS16 v2.4]\033[0m %s\n' "$*"; }
warn(){ printf '\033[1;33m[CS16 v2.4 WARNING]\033[0m %s\n' "$*" >&2; }
...
say 'Installing real-engine HLDS runtime and self-healing monitor...'
...
if [[ -z "$SERVER_IDS" ]]; then
  ...
  for SID in $SERVER_IDS; do
    say "Server #$SID: verified restart + automatic recovery if needed"
    TMP="$(mktemp)"
    if timeout 150 /usr/local/sbin/hyper-cs16-ctl restart "$SID" >"$TMP" 2>&1; then
      cat "$TMP"
    else
...
try: import pymysql
    ...
    print('[CS16 v2.4] PyMySQL unavailable:',exc); raise SystemExit(0)
...
for sid in ids:
    try:
        ...
        print(f'[CS16 v2.4] #{sid}: {state} real_pid={data.get("pid",0)} UDP={udp} A2S={data.get("query_state","-")}')
    except Exception as exc:
        print(f'[CS16 v2.4] #{sid}: status refresh warning: {exc}')
with con.cursor() as cur:
    cur.execute("INSERT INTO settings(setting_key,setting_value) VALUES('panel_version','2.4.0') ON DUPLICATE KEY UPDATE setting_value=VALUES(setting_value)")
con.close()
PY
...
echo
if [[ "$FAIL" -eq 0 ]]; then
  say 'DONE. Existing servers were migrated and health-checked.'
else
...
say 'The panel now runs hlds_linux directly, auto-recovers broken restarts/maps, and never treats hlds_run sleep as a healthy server.'
