...
BACKUP="/root/hyper-cs16-fastdl-v10-${STAMP}"
...
echo "================================================================"
echo " HYPER-HOST CS 1.6 FASTDL IP FINAL v10"
echo " Server:     #$SID"
...
echo "[2/11] Quarantine recursive nginx backup garbage..."
...
for root in roots:
    ...
    for p in list(root.iterdir()):
        ...
        target=out/(root.as_posix().strip('/').replace('/','__')+'__'+p.name)
        i=1
        while target.exists() or target.is_symlink():
            target=out/(root.as_posix().strip('/').replace('/','__')+f'__{p.name}.{i}'); i+=1
        try:
            if p.is_symlink():
...
marker='# HYPER-HOST FASTDL IP FINAL v10'
...
for p in (runtime,state):
    ...
    t=p.with_name('.'+p.name+'.v10.tmp'); t.write_text(json.dumps(d,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); t.replace(p)
    print('updated:',p)
PY
...
for p in loaded:
    ...
    # HYPER-HOST FASTDL IP FINAL v10
    location ^~ /fastdl/ {{
        ...
        add_header X-Hyper-FastDL "raw-v10" always;
    ...
    location / {{ return 301 https://{domain}$request_uri; }}
            ...
            ins='''
    # HYPER-HOST FASTDL IP FINAL v10 BEGIN
    location ^~ /fastdl/ {
        alias /srv/hyper-cs16/fastdl/;
        autoindex off;
        sendfile on;
    ...
    # HYPER-HOST FASTDL IP FINAL v10 END
'''
            x=new.find('{')+1; new=new[:x]+ins+new[x:]; reps.append((a,b,new))
    if reps:
        safe=p.as_posix().strip('/').replace('/','__'); before=backup/(safe+'.before-v10')
...
echo
echo "================================================================"
echo " [SUCCESS] HYPER-HOST FASTDL IP FINAL v10"
echo "================================================================"
