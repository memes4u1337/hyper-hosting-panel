...
# HYPER CLOUD v104 — standalone domain hard-fix / multi-user / shared storage / chunk uploads.
# Usage: sudo ./apply-v2.2-cloud-vhost-fix.sh [/root/hyper-hosting-panel]
...
BACKUP="/opt/hyper-host/backups/cloud-v104-${STAMP}"
...
s=re.sub(r"function hh_app_version\(\): string \{ return '[^']+'; \}", "function hh_app_version(): string { return '1.9-v104-cloud'; }", s)
...
if [[ "$ROUTE" != "HYPER_CLOUD_V104" ]]; then
...
printf '[HYPER CLOUD] DOMAIN_OK: %s://%s/ -> HYPER CLOUD v104\n' "$PROTO" "$CLOUD_DOMAIN"

PUBLIC_IP="$(curl -4 -fsS --max-time 5 https://api.ipify.org 2>/dev/null || true)"
printf '\n\033[1;32m============================================================\033[0m\n'
printf '\033[1;32m HYPER CLOUD v104 установлен\033[0m\n'
printf ' URL: https://%s/\n' "$CLOUD_DOMAIN"
