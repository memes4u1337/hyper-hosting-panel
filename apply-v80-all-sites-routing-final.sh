...
BACKUP="$BASE/backups/v80-all-sites-routing-$STAMP"
REPORT=/root/hyper-host-v80-all-sites-routing-report.txt
PLAN="$BASE/data/site-routing-plan.txt"
INDEX_MANIFEST_BEFORE="/tmp/hyper-host-v80-index-before-$$.json"
INDEX_MANIFEST_AFTER="/tmp/hyper-host-v80-index-after-$$.json"
mkdir -p "$BACKUP"
...
ensure_nginx_writable(){
  local test=/etc/nginx/hyper-host-v80-write-test-$$ runtime=/opt/hyper-host/runtime/nginx mount_script=/opt/hyper-host/bin/mount-nginx-runtime.sh
...
log 'Исправляю маршрутизацию всех сайтов. Файлы сайтов, панель, FTP, SQL, боты и admin не изменяются.'
...
RESULTS="/tmp/hyper-host-v80-results-$$.txt"
: > "$RESULTS"
TOTAL_SITES=0
TOTAL_HOSTS=0
HTTP_OK=0
...
while IFS='|' read -r domain aliases phpv; do
  ...
  [[ -f "$conf" ]] || fail "Нет vhost для $domain: $conf"
  grep -qF "root $root;" "$conf" || fail "$domain смотрит не в свой public_html"

  probe="hyper-host-v80-probe-${STAMP}-${TOTAL_SITES}-$$.txt"
  body="HYPER-HOST-V80-${domain}-${STAMP}-$$"
...
UNKNOWN="v80-unknown-${STAMP}-$$.invalid"
UNKNOWN_BODY="$(curl --noproxy '*' -sS --connect-timeout 2 --max-time 5 -H "Host: $UNKNOWN" "http://$SERVER_IP/" 2>/dev/null || true)"
[[ "$UNKNOWN_BODY" != *'HYPER-HOST-V80-'* ]] || fail 'Неизвестный домен попал в один из сайтов'
...
{
  echo 'HYPER-HOST v80 — all sites routing final'
  echo
...
printf '\n%s\n' '============================================================'
printf '%s\n' ' HYPER-HOST — все сайты привязаны к своим public_html'
printf '%s\n' '============================================================'
printf ' Сайтов:             %s\n' "$TOTAL_SITES"
printf ' Доменов/aliases:    %s\n' "$TOTAL_HOSTS"
