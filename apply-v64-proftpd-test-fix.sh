...
BACKUP_DIR="$BASE_DIR/backups/v64-proftpd-test-$(date +%Y%m%d-%H%M%S)"
REPORT=/root/hyper-host-v64-proftpd-report.txt
PROJECT='HYPER-HOST'
CYAN='\033[1;36m'
RESET='\033[0m'
ROLLBACK_NEEDED=1
TEST_USER="hhv64$(date +%s)"
TEST_PASS="V64$(openssl rand -hex 16)"
TEST_REMOTE="v64-gnutls-data-$(date +%s)-$(openssl rand -hex 4).bin"
TEST_TMP_DIR="$(mktemp -d /tmp/hhv64-test.XXXXXX)"
TEST_SRC="$TEST_TMP_DIR/source.bin"
TEST_DST="$TEST_TMP_DIR/downloaded.bin"
LFTP_LOG="$TEST_TMP_DIR/lftp.log"
...
cleanup_test_user() {
  ...
  rm -f /tmp/hhv64-deleted-login.log >/dev/null 2>&1 || true
}
...
log 'Исправляю только проверку установки ProFTPD/FTPS. Nginx, SQL, сайты и пароль admin не изменяются.'
...
printf 'HYPER-HOST v64 ProFTPD GnuTLS data-channel test\n%.0s' {1..4096} > "$TEST_SRC"
rm -f "$TEST_DST" "$LFTP_LOG"
...
[[ -f "$TEST_DST" ]] || fail 'lftp не создал скачанный файл'
SRC_SHA="$(sha256sum "$TEST_SRC" | awk '{print $1}')"
DST_SHA="$(sha256sum "$TEST_DST" | awk '{print $1}')"
[[ "$SRC_SHA" == "$DST_SHA" ]] || fail 'SHA-256 скачанного через FTPS файла не совпадает с исходным' 
...
sleep 1
if timeout 25 lftp -u "$TEST_USER,$TEST_PASS" 'ftp://127.0.0.1:21' >/tmp/hhv64-deleted-login.log 2>&1 <<'LFTPDELETE'
set cmd:fail-exit true
set ssl:verify-certificate no
set ftp:ssl-force true
set ftp:ssl-protect-data true
...
LFTPDELETE
then
  cat /tmp/hhv64-deleted-login.log >&2 || true
  fail 'Удалённый FTP-аккаунт всё ещё может войти'
fi
rm -f /tmp/hhv64-deleted-login.log
...
{
  echo 'HYPER-HOST v64 ProFTPD FTPS installer-test fix'
