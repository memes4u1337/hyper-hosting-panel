...
PATCH_VERSION="v100-cloud-editor"
...
BACKUP_DIR="${BACKUP_BASE}/cloud-v100-${STAMP}"
...
[[ "$EUID" -eq 0 ]] || fail "Запусти от root: sudo ./apply-v1.8-cloud-editor.sh /root/hyper-hosting-panel"
...
# Version marker.
s,n=re.subn(r"function hh_app_version\(\): string \{ return '[^']+'; \}", "function hh_app_version(): string { return '1.9-v100-cloud'; }", s, count=1)
if n!=1:
...
log "Добавляю просмотр и редактирование ZIP/7z (если пакеты доступны)..."
