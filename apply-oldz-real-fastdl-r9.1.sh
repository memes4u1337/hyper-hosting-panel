...
mkdir -p "$BACKUP"
echo "=============================================================="
echo " OLD ZOMBIE REAL FASTDL R9.1"
echo " Server:  #${SID}"
echo " cstrike: ${CS}"
echo " Backup:  ${BACKUP}"
echo "=============================================================="
...
echo "[4/10] Fixing M82 missing WAV files from the installed build..."
mkdir -p "$CS/sound/weapons"
M82_FIXED=0
M82_MISSING=0
for name in m82-1.wav m82_clipin1.wav m82_clipin2.wav m82_clipout1.wav m82_clipout2.wav; do
    dst="$CS/sound/weapons/$name"
    if [[ -s "$dst" ]]; then
        echo "[OK] already present: sound/weapons/$name"
        continue
    fi
    ...
    if [[ -z "$src" ]]; then
        src="$(find "$CS" -maxdepth 4 -type f -iname "$name" ! -path "$dst" -print -quit 2>/dev/null || true)"
    fi
    ...
    if [[ -n "$src" && -s "$src" ]]; then
        ...
        echo "[OK] $src -> $dst"
        M82_FIXED=$((M82_FIXED+1))
    else
        echo "[WARN] M82 resource not found anywhere in cstrike: $name"
        M82_MISSING=$((M82_MISSING+1))
    fi
done
echo "[INFO] M82 copied=$M82_FIXED missing=$M82_MISSING"
...
echo
echo "=============================================================="
echo "[SUCCESS] OLD ZOMBIE REAL FASTDL R9.1 APPLIED"
echo
