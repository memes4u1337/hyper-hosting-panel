...
BACKUP_DIR="$BASE_DIR/backups/v63-proftpd-ubuntu-$(date +%Y%m%d-%H%M%S)"
REPORT=/root/hyper-host-v63-proftpd-report.txt
PROJECT='HYPER-HOST'
CYAN='\033[1;36m'
RESET='\033[0m'
ROLLBACK_NEEDED=1
TEST_USER="hhv63$(date +%s)"
TEST_PASS="V63$(openssl rand -hex 16)"
TEST_REMOTE="v63-gnutls-data-$(date +%s).bin"
TEST_SRC="$(mktemp /tmp/hhv63-src.XXXXXX)"
TEST_DST="$(mktemp /tmp/hhv63-dst.XXXXXX)"
LFTP_LOG="$(mktemp /tmp/hhv63-lftp.XXXXXX)"
...
cleanup_test_user() {
  ...
  rm -f "$TEST_SRC" "$TEST_DST" "$LFTP_LOG" /tmp/hhv63-deleted-login.log >/dev/null 2>&1 || true
}
...
log 'Исправляю только конфиг ProFTPD для Ubuntu 22.04. Nginx, SQL, сайты и пароль admin не изменяются.'
...
printf 'HYPER-HOST v63 ProFTPD GnuTLS data-channel test\n%.0s' {1..4096} > "$TEST_SRC"
...
sleep 1
if timeout 25 lftp -u "$TEST_USER,$TEST_PASS" 'ftp://127.0.0.1:21' >/tmp/hhv63-deleted-login.log 2>&1 <<'LFTPDELETE'
set cmd:fail-exit true
set ssl:verify-certificate no
set ftp:ssl-force true
set ftp:ssl-protect-data true
...
LFTPDELETE
then
  cat /tmp/hhv63-deleted-login.log >&2 || true
  fail 'Удалённый FTP-аккаунт всё ещё может войти'
fi
rm -f /tmp/hhv63-deleted-login.log
...
{
  echo 'HYPER-HOST v63 ProFTPD FTPS fix'
