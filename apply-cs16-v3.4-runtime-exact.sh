...
[[ -f "$SRC/bin/hyper-cs16-ctl" && -f "$SRC/bin/hyper-cs16-monitor" && -f "$SRC/public/index.php" && -f "$SRC/app/bootstrap.php" ]] || {
  echo '[ERROR] Run this script from the v3.4-runtime-exact patched HYPER-HOST repository root' >&2; exit 2;
}
BASE='/opt/hyper-cs16'
DOMAIN='www.avito.hyper-host.pw'
CANONICAL_PUBLIC='/var/www/hyper-host-sites/www.avito.hyper-host.pw/public_html'

say(){ printf '\033[1;36m[CS16 v3.4-runtime-exact]\033[0m %s\n' "$*"; }
warn(){ printf '\033[1;33m[CS16 v3.4-runtime-exact WARNING]\033[0m %s\n' "$*" >&2; }
fail(){ printf '\033[1;31m[CS16 v3.4-runtime-exact ERROR]\033[0m %s\n' "$*" >&2; exit 1; }
...
say 'No bundled game/server build files are included in this patch. Full Linux cstrike runtimes are now preserved from user uploads.'
...
say 'Leaving all existing game-server files/configs untouched.'
say 'This patch only updates panel/controller code. SQL/FastDL for an existing server changes only on an explicit action or a new build install.'

say 'Creating legacy MySQL socket compatibility for old AMXX plugins (no game files changed)...'
MYSQL_SOCKET=''
for c in /run/mysqld/mysqld.sock /var/run/mysqld/mysqld.sock /var/lib/mysql/mysql.sock; do
  [[ -S "$c" ]] && { MYSQL_SOCKET="$c"; break; }
done
if [[ -n "$MYSQL_SOCKET" ]]; then
  if [[ -S /tmp/mysql.sock && ! -L /tmp/mysql.sock ]]; then
    say 'Legacy MySQL socket already exists at /tmp/mysql.sock; leaving it untouched.'
  else
    rm -f /tmp/mysql.sock 2>/dev/null || true
    ln -s "$MYSQL_SOCKET" /tmp/mysql.sock
    cat >/etc/tmpfiles.d/hyper-cs16-mysql-compat.conf <<EOF
L+ /tmp/mysql.sock - - - - $MYSQL_SOCKET
EOF
    say "Legacy MySQL socket ready: /tmp/mysql.sock -> $MYSQL_SOCKET"
  fi
else
  warn 'MariaDB socket was not found; /tmp/mysql.sock compatibility was not created.'
fi
...
say "Deploying v3.4-runtime-exact to ${#ROOTS[@]} detected document root(s)..."
for DOCROOT in "${!ROOTS[@]}"; do
  ...
  grep -q "HYPER_CS16_PANEL_BUILD = '3.3-fullbuild'" "$APPDIR/bootstrap.php" || fail "Wrong bootstrap deployed to $APPDIR/bootstrap.php"
  ...
  grep -q 'style.css?v=340' "$DOCROOT/index.php" || fail "Wrong index.php deployed to $DOCROOT"
...
say 'Existing server.cfg files were not modified; FastDL will be applied on the next build install or explicit FastDL sync.'
...
say 'Verifying that nginx serves v3.4-runtime-exact, not cached/old PHP...'
LIVE="$(curl -ksS --max-time 15 --resolve "$DOMAIN:443:127.0.0.1" "https://$DOMAIN/?page=login" 2>/dev/null || true)"
if [[ -n "$LIVE" ]]; then
  if grep -q 'style.css?v=340' <<<"$LIVE"; then
    say 'Live web verification OK: panel v3.4-runtime-exact is being served.'
  else
    warn 'Live HTTPS answered, but v3.4-runtime-exact marker was not found. Check nginx root/server_name; file deployment itself passed.'
  fi
else
...
echo
say 'DONE. Exact runtime build import + transactional rollback + FastDL/SQL support are installed. Existing game servers were not modified. No game/server build files are bundled.'
say 'Open the panel and press Ctrl+F5. The sidebar must show Panel v3.4-runtime-exact.'
