...
[[ -f "$SRC/bin/hyper-cs16-ctl" && -f "$SRC/bin/hyper-cs16-monitor" && -f "$SRC/public/index.php" && -f "$SRC/app/bootstrap.php" ]] || {
  echo '[ERROR] Run this script from the v3.0.3-lite patched HYPER-HOST repository root' >&2; exit 2;
}
BASE='/opt/hyper-cs16'
DOMAIN='www.avito.hyper-host.pw'
CANONICAL_PUBLIC='/var/www/hyper-host-sites/www.avito.hyper-host.pw/public_html'

say(){ printf '\033[1;36m[CS16 v3.0.3-lite]\033[0m %s\n' "$*"; }
warn(){ printf '\033[1;33m[CS16 v3.0.3-lite WARNING]\033[0m %s\n' "$*" >&2; }
fail(){ printf '\033[1;31m[CS16 v3.0.3-lite ERROR]\033[0m %s\n' "$*" >&2; exit 1; }
...
say "Deploying v3.0.3-lite to ${#ROOTS[@]} detected document root(s)..."
for DOCROOT in "${!ROOTS[@]}"; do
  ...
  grep -q "HYPER_CS16_PANEL_BUILD = '3.0.3-lite'" "$APPDIR/bootstrap.php" || fail "Wrong bootstrap deployed to $APPDIR/bootstrap.php"
  ...
  grep -q 'style.css?v=303' "$DOCROOT/index.php" || fail "Wrong index.php deployed to $DOCROOT"
...
say 'Verifying that nginx serves v3.0.3-lite, not cached/old PHP...'
LIVE="$(curl -ksS --max-time 15 --resolve "$DOMAIN:443:127.0.0.1" "https://$DOMAIN/?page=login" 2>/dev/null || true)"
if [[ -n "$LIVE" ]]; then
  if grep -q 'style.css?v=303' <<<"$LIVE"; then
    say 'Live web verification OK: panel v3.0.3-lite is being served.'
  else
    warn 'Live HTTPS answered, but v3.0.3-lite marker was not found. Check nginx root/server_name; file deployment itself passed.'
  fi
else
...
echo
say 'DONE. Custom ZIP/RAR installs now normalize Windows CRLF configs, keep the Linux AMXX core.ini, adopt build server.cfg/hostname, and verify ZM at runtime. No game/server build files are bundled.'
say 'Open the panel and press Ctrl+F5. The sidebar must show Panel v3.0.3-lite.'
