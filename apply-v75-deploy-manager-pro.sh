...
BACKUP="$BASE/backups/v75-deploy-manager-$STAMP"
REPORT=/root/hyper-host-v75-deploy-manager-report.txt
...
mkdir -p "$BACKUP" "$BASE/deploy-center/examples" "$MASTER" "$TEMPLATE" "$MANAGED"
...
printf '%s\n' '[HYPER-HOST] Устанавливаю отдельную страницу Deploy Manager и новый серверный менеджер.'
...
install -m 0644 "$ROOT/ready-master-bot/bot.py" "$BASE/deploy-center/examples/bot.py"
install -m 0644 "$ROOT/ready-master-bot/requirements.txt" "$BASE/deploy-center/examples/requirements.txt"
install -m 0600 "$ROOT/ready-master-bot/.env.example" "$BASE/deploy-center/examples/.env.example"
...
cat > "$REPORT" <<EOF
HYPER-HOST v75 — separate MyStock Deploy Manager

Page: /?page=deploy_center
Master files: $MASTER/{bot.py,.env,requirements.txt}
Store template: $TEMPLATE/{bot.py,requirements.txt}
Managed projects: $MANAGED/<project_id>-<project_name>/
Ready rewritten master bot: $BASE/deploy-center/examples/

SQL sync: $SYNC_STATUS
Doctor: $DOCTOR
Removed untouched v74 placeholders: ${REMOVED_PLACEHOLDERS[*]:-none}

SSL was NOT changed by v75.
