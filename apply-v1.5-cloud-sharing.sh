...
PATCH_VERSION="v97-cloud-private-sharing"
...
BACKUP_DIR="${BACKUP_BASE}/cloud-v97-${STAMP}"
...
[[ "$EUID" -eq 0 ]] || fail "Запусти от root: sudo ./apply-v1.5-cloud-sharing.sh /root/hyper-hosting-panel"
[[ -f "$PROJECT_DIR/src/public/index.php" ]] || fail "Не найден src/public/index.php в $PROJECT_DIR"
...
python3 - "$PROJECT_DIR/src/public/index.php" <<'PY'
...
# Version marker.
s,n=re.subn(r"function hh_app_version\(\): string \{ return '[^']+'; \}", "function hh_app_version(): string { return '1.9-v97-cloud'; }", s, count=1)
if n!=1:
...
log "Добавляю просмотр ZIP/RAR/7z (если пакеты доступны)..."
if command -v apt-get >/dev/null 2>&1; then
  DEBIAN_FRONTEND=noninteractive apt-get install -y p7zip-full libarchive-tools unzip >/dev/null 2>&1 || warn "Архиваторы не установились автоматически. Сам Cloud работать будет; просмотр RAR/7z можно поставить позже."
fi
...
log "Все файлы приватны по умолчанию. Публичны только файлы с включённой ссылкой."
