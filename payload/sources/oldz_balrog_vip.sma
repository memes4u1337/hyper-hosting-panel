...
#define CrossBow_WEAPONKEY 		8146
...
#define CrossBow_RELOAD_TIME	3.7
#define CrossBow_SHOOT1			4
#define CrossBow_SHOOT2			3
#define CrossBow_RELOAD			1
#define CrossBow_DRAW			2

#define write_coord_f(%1)	engfunc(EngFunc_WriteCoord,%1)

new const Fire_Sounds[][] = { "oldz_w3/bal_fire.wav" }

new CrossBow_V_MODEL[64] = "models/oldz_w3/balrog_v.mdl"
new CrossBow_P_MODEL[64] = "models/oldz_w3/balrog_p.mdl"
new CrossBow_W_MODEL[64] = "models/oldz_w3/balrog_w.mdl"

new const GUNSHOT_DECALS[] = { 41, 42, 43, 44, 45 }

new cvar_dmg_CrossBow, cvar_recoil_CrossBow, cvar_clip_CrossBow, cvar_spd_CrossBow, cvar_CrossBow_ammo
new g_MaxPlayers, g_orig_event_CrossBow, g_IsInPrimaryAttack
new Float:cl_pushangle[MAX_PLAYERS + 1][3], m_iBlood[2]
new g_has_CrossBow[33], g_clip_ammo[33], g_CrossBow_TmpClip[33], oldweap[33]
new gmsgWeaponList
...
public plugin_init()
{
	register_plugin("OLD ZOMBIE - Balrog Crossbow VIP", "2.0", "OLD ZOMBIE")
	register_clcmd("oldz_give_balrog", "cmd_oldz_give")
	...
	RegisterHam(Ham_Item_PostFrame, "weapon_sg550", "CrossBow_ItemPostFrame")
	RegisterHam(Ham_Weapon_Reload, "weapon_sg550", "CrossBow_Reload")
	RegisterHam(Ham_Weapon_Reload, "weapon_sg550", "CrossBow_Reload_Post", 1)
	...
	cvar_dmg_CrossBow = register_cvar("zp_CrossBow_dmg", "2.0")
	cvar_recoil_CrossBow = register_cvar("zp_CrossBow_recoil", "0.61")
	cvar_clip_CrossBow = register_cvar("zp_CrossBow_clip", "50")
	cvar_spd_CrossBow = register_cvar("zp_CrossBow_spd", "0.7")
	cvar_CrossBow_ammo = register_cvar("zp_CrossBow_ammo", "150")
...
public plugin_precache()
{
	...
	precache_sound("oldz_w3/bal_out.wav")
	precache_sound("oldz_w3/bal_in.wav")
	precache_sound("oldz_w3/bal_out.wav")
	precache_sound("oldz_w3/bal_in.wav")
	precache_sound("oldz_w3/bal_draw.wav")
...
{
	register_native("give_weapon_CrossBow", "native_give_weapon_add", 1)
}
...
public give_CrossBow(id)
{
	...
	if( iWep2 > 0 )
	{
		...
		UTIL_PlayWeaponAnimation(id, CrossBow_DRAW)
...
public cmd_oldz_give(id)
{
	...
	give_CrossBow(id);
	oldz_weapon_chat(id, "Вы получили Balrog Crossbow [VIP].");
	return PLUGIN_HANDLED;
}
...
public fw_CrossBow_PrimaryAttack_Post(Weapon)
{
	...
	if(g_has_CrossBow[Player])
	{
		...
		UTIL_PlayWeaponAnimation(Player, random_num(CrossBow_SHOOT1, CrossBow_SHOOT2))
	}
}
...
public CrossBow_ItemPostFrame(weapon_entity) 
{
...
public CrossBow_Reload(weapon_entity) 
{
     new id = pev(weapon_entity, pev_owner)
     if (!is_user_connected(id))
          return HAM_IGNORED
...
public CrossBow_Reload_Post(weapon_entity) 
{
	new id = pev(weapon_entity, pev_owner)
	if (!is_user_connected(id))
		return HAM_IGNORED
