...
#define OLDZ_WPN_AKBLOOD 2
native zv_get_user_flags(id)
native oldz_has_zp_admin_access(id)
native oldz_vip_weapon_try_take(id, weapon_id)
...
public fw_SetModel(entity, model[])
{
	...
	if(iOwner < 1 || iOwner > g_MaxPlayers || !is_user_connected(iOwner))
		return FMRES_IGNORED;
...
public cmd_oldz_give(id)
{
    ...
    if (!oldz_vip_weapon_try_take(id, OLDZ_WPN_AKBLOOD))
        return PLUGIN_HANDLED
...
{
	if(Player < 1 || Player > g_MaxPlayers || !is_user_connected(Player))
		return FMRES_IGNORED
...
public fwPlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	if (eventid != g_orig_event_ak47 || !g_IsInPrimaryAttack)
		return FMRES_IGNORED
	if (invoker < 1 || invoker > g_MaxPlayers || !is_user_connected(invoker))
		return FMRES_IGNORED
