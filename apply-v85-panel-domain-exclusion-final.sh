...
BACKUP="$BASE/backups/v85-panel-domain-exclusion-final-$STAMP"
REPORT=/root/hyper-host-v85-panel-domain-exclusion-final-report.txt
ROUTING_MAP="$BASE/data/v85-routing.tsv"
INDEX_BEFORE="/tmp/hyper-host-v85-index-before-$$.json"
INDEX_AFTER="/tmp/hyper-host-v85-index-after-$$.json"
mkdir -p "$BACKUP"
...
ensure_nginx_writable(){
  local test="/etc/nginx/hyper-host-v85-write-test-$$" runtime=/opt/hyper-host/runtime/nginx mount_script=/opt/hyper-host/bin/mount-nginx-runtime.sh
...
[[ -f /opt/hyper-host/nginx_recover_v85.py ]] && cp -a /opt/hyper-host/nginx_recover_v85.py "$BACKUP/nginx_recover_v85.py"
[[ -f /opt/hyper-host/nginx_conflict_resolver_v85.py ]] && cp -a /opt/hyper-host/nginx_conflict_resolver_v85.py "$BACKUP/nginx_conflict_resolver_v85.py"
...
rollback(){
  local code=$?
  trap - ERR
  printf '[HYPER-HOST] Ошибка. Возвращаю Nginx и CLI в состояние до v85.\n' >&2
  ...
  if [[ -f "$BACKUP/nginx_recover_v85.py" ]]; then
    cp -a "$BACKUP/nginx_recover_v85.py" /opt/hyper-host/nginx_recover_v85.py
  else
    rm -f /opt/hyper-host/nginx_recover_v85.py
  fi
  if [[ -f "$BACKUP/nginx_conflict_resolver_v85.py" ]]; then
    cp -a "$BACKUP/nginx_conflict_resolver_v85.py" /opt/hyper-host/nginx_conflict_resolver_v85.py
  else
    rm -f /opt/hyper-host/nginx_conflict_resolver_v85.py
  fi
...
[[ "$BETA_DOMAIN" != "$PANEL_DOMAIN" ]] || fail "Домен beta совпадает с доменом панели: $PANEL_DOMAIN"
...
log 'Восстанавливаю один точный vhost панели и отдельные vhost сайтов. Папка сайта с именем домена панели сохраняется, но не включается в Nginx.'
install -m 0755 "$ROOT/scripts/hhctl" /usr/local/sbin/hyper-host-ctl
install -m 0755 "$ROOT/scripts/nginx_recover_v85.py" /opt/hyper-host/nginx_recover_v85.py
install -m 0755 "$ROOT/scripts/nginx_conflict_resolver_v85.py" /opt/hyper-host/nginx_conflict_resolver_v85.py
bash -n /usr/local/sbin/hyper-host-ctl
python3 -W error -m py_compile /opt/hyper-host/nginx_recover_v85.py /opt/hyper-host/nginx_conflict_resolver_v85.py
...
python3 /opt/hyper-host/nginx_recover_v85.py \
  ...
  --map "$ROUTING_MAP" \
  --cleanup-report "$BASE/data/v85-nginx-cleanup.txt"

# Resolve any duplicate host that survived under an unexpected filename/include.
# The canonical v85 route is kept; the exact duplicate token is removed only from
# the other active config. Full /etc/nginx is already backed up above.
CONFLICT_REPAIR_LOG="$BASE/data/v85-conflict-repair.txt"
: > "$CONFLICT_REPAIR_LOG"
for round in 1 2 3 4 5; do
  NGINX_TEST="$(nginx -t 2>&1 || true)"
  printf '%s\n' "$NGINX_TEST"
  mapfile -t CONFLICT_HOSTS < <(printf '%s\n' "$NGINX_TEST" | sed -nE 's/.*conflicting server name "([^"]+)".*/\1/p' | sort -u)
  if [[ ${#CONFLICT_HOSTS[@]} -eq 0 ]]; then
    printf '%s' "$NGINX_TEST" | grep -q 'test is successful' || fail 'Nginx не прошёл синтаксическую проверку.'
    break
  fi
  for conflict_host in "${CONFLICT_HOSTS[@]}"; do
    keep_conf=""
    case "$conflict_host" in
      "$PANEL_DOMAIN"|"$LAN_IP"|"$PUBLIC_IP"|localhost)
        keep_conf=/etc/nginx/sites-enabled/01-hyper-host-panel.conf
        ;;
      *)
        owner_domain="$(awk -F '\t' -v h="$conflict_host" '$1==h {print $2; exit}' "$ROUTING_MAP")"
        [[ -n "$owner_domain" ]] && keep_conf="/etc/nginx/sites-enabled/20-hyper-host-site-$owner_domain.conf"
        ;;
    esac
    [[ -n "$keep_conf" && -e "$keep_conf" ]] || fail "Не найден канонический vhost для конфликтующего имени $conflict_host"
    python3 /opt/hyper-host/nginx_conflict_resolver_v85.py --host "$conflict_host" --keep "$keep_conf" | tee -a "$CONFLICT_REPAIR_LOG"
  done
done
NGINX_TEST="$(nginx -t 2>&1)"
printf '%s\n' "$NGINX_TEST"
[[ "$NGINX_TEST" != *'conflicting server name'* ]] || { nginx -T 2>&1 | grep -E 'conflicting server name|configuration file|server_name' | tail -n 200 >&2 || true; fail 'Не удалось убрать все конфликтующие server_name.'; }
systemctl reload nginx >/dev/null 2>&1 || systemctl restart nginx
sleep 1

probe_route(){
  local host="$1" root="$2" label="$3" probe body response="" attempt
  probe="hyper-host-v85-${label}-${STAMP}-$$.txt"
  body="HYPER-HOST-V85-${label}-${STAMP}-$$"
...
PANEL_PROBE="hyper-host-v85-panel-${STAMP}-$$.txt"
PANEL_BODY="HYPER-HOST-V85-PANEL-${STAMP}-$$"
...
PANEL_SITE_FOLDER="/var/www/hyper-host-sites/$PANEL_DOMAIN/public_html"
PANEL_SITE_FOLDER_STATE="отсутствует"
[[ -d "$PANEL_SITE_FOLDER" ]] && PANEL_SITE_FOLDER_STATE="сохранена на диске, исключена из Nginx"

[[ -s "$ROUTING_MAP" ]] || fail "Карта сайтов не создана: $ROUTING_MAP"
if awk -F '\t' -v panel="$PANEL_DOMAIN" '$1==panel || $2==panel {found=1} END{exit !found}' "$ROUTING_MAP"; then
  fail "Домен панели $PANEL_DOMAIN ошибочно попал в карту сайтов."
fi
TOTAL=0; OK=0; BETA_OK=0
RESULTS="/tmp/hyper-host-v85-results-$$.txt"; : > "$RESULTS"
...
{
  echo 'HYPER-HOST v85 — panel domain reserved; sites and beta routed correctly'
  echo
  ...
  echo "Panel-domain site folder: $PANEL_SITE_FOLDER_STATE"
...
printf ' Папка panel-site:    %s\n' "$PANEL_SITE_FOLDER_STATE"
