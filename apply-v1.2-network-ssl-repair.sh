...
BACKUP_DIR="$BASE_DIR/backups/v1.2-network-ssl-repair-$TIMESTAMP"
REPORT="$BASE_DIR/logs/network-ssl-repair-$TIMESTAMP.txt"
...
OLD_PUBLIC_IP=""
if [[ -f "$CONF" ]]; then
  OLD_PUBLIC_IP="$(python3 - "$CONF" <<'PYOLDIP'
...
for line in text.splitlines():
    match = re.match(r'\s*PUBLIC_IP\s*=\s*[\"\']?([^\"\'\s#]+)', line)
    if match:
...
: > "$REPORT"
{
  echo 'HYPER-HOST v1.2 — NETWORK / SSL REPAIR REPORT'
  echo "LAN_IP=$LAN_IP"
...
if [[ -d /var/www/hyper-host-sites ]]; then
  while IFS= read -r -d '' site_dir; do
    ...
    a="$(dig +short @1.1.1.1 A "$domain" 2>/dev/null | awk '/^([0-9]{1,3}\.){3}[0-9]{1,3}$/' | sort -u | paste -sd, - || true)"
    aaaa="$(dig +short @1.1.1.1 AAAA "$domain" 2>/dev/null | awk '/:/' | sort -u | paste -sd, - || true)"
    printf '%s\tA=%s\tAAAA=%s\n' "$domain" "${a:-NONE}" "${aaaa:-NONE}" >> "$REPORT"
  done < <(find /var/www/hyper-host-sites -mindepth 1 -maxdepth 1 -type d -print0)
fi

# Во время патча не запускаем сетевое продление вслепую.
# Существующие сертификаты уже восстановлены и подключены выше; продление выполняет cron.
"$HYPER_BIN" ssl status >>"$REPORT" 2>&1 || true
