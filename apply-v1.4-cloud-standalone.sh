...
PATCH_VERSION="v96-cloud-standalone"
...
BACKUP_DIR="${BACKUP_BASE}/cloud-standalone-${STAMP}"
...
[[ "$EUID" -eq 0 ]] || fail "Запусти от root: sudo ./apply-v1.4-cloud-standalone.sh /root/hyper-hosting-panel"
...
# Version marker.
s,n=re.subn(r"function hh_app_version\(\): string \{ return '[^']+'; \}", "function hh_app_version(): string { return '1.9-v96-cloud'; }", s, count=1)
if n!=1:
...
log "Без авторизации /cloud/ перенаправляет на вход в HYPER-HOST."
