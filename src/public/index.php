...
function view_ssl(): void {
    ...
    $certs=run_ctl_json(['ssl-status-json'],30); $map=[]; if(!isset($certs['_error'])) foreach($certs as $c) $map[$c['domain']]=$c;
...
<div class="panel-card mb-4">
  ...
  <?php foreach($sites as $s):
      $c=$map[$s['domain']]??null;
      $dns=run_ctl_json(['ssl-check-json',$s['domain']],20);
  ...
  ?>
    <tr>
      ...
      <td><button class="btn btn-sm btn-soft" data-bs-toggle="modal" data-bs-target="#ssl<?= (int)$s['id'] ?>">Выпустить</button></td>
    </tr>
