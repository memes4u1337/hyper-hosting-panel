...
function handle_post(string $action): void
{
    try {
        switch ($action) {
            ...
            case 'repair_panel': { $res=run_ctl(['repair'],240); if($res['code']!==0) throw new RuntimeException($res['output']); flash('Ремонт выполнен: права, ACL, FTP, сервисы проверены','success'); redirect('/?page=settings'); }
            case 'sync_resources': { sync_resources(); flash('Ресурсы синхронизированы','success'); redirect('/?page=dashboard'); }
...
{ $stats=run_ctl_json(['stats-json'],40); $events=db()->query('SELECT * FROM events ORDER BY id DESC LIMIT 8')->fetchAll(); ?>
...
{ $sites=db()->query('SELECT * FROM sites ORDER BY id DESC')->fetchAll(); $folders=db()->query('SELECT * FROM folders ORDER BY id DESC')->fetchAll(); $php=run_ctl_json(['php-list-json'],30); ?>
...
function view_dns(): void { $zones=db()->query('SELECT * FROM dns_zones ORDER BY id DESC')->fetchAll(); ?>
<div class="row g-4"><div class="col-lg-4"><div class="panel-card"><h2>DNS зона</h2><form method="post" class="vstack gap-3"><?= csrf_field() ?><input type="hidden" name="action" value="create_dns_zone"><input class="form-control" name="domain" placeholder="example.com" required><input class="form-control" name="primary_ns" value="ns1.example.com."><input class="form-control" name="admin_email" value="admin.example.com."><button class="btn btn-primary">Создать/обновить зону</button></form><p class="muted mt-3">Работает, если домены обслуживаются на твоих NS и установлен bind9.</p></div></div><div class="col-lg-8"><?php foreach($zones as $z): $rs=db()->prepare('SELECT * FROM dns_records WHERE zone_id=? ORDER BY id'); $rs->execute([(int)$z['id']]); $recs=$rs->fetchAll(); ?><div class="panel-card mb-4"><div class="card-title-row"><h2><?= e($z['domain']) ?></h2><form method="post" onsubmit="return confirm('Удалить DNS зону?')"><?= csrf_field() ?><input type="hidden" name="action" value="delete_dns_zone"><input type="hidden" name="id" value="<?= (int)$z['id'] ?>"><button class="btn btn-sm btn-outline-danger">Удалить</button></form></div><form method="post" class="row g-2 mb-3"><?= csrf_field() ?><input type="hidden" name="action" value="add_dns_record"><input type="hidden" name="zone_id" value="<?= (int)$z['id'] ?>"><div class="col-md-2"><select class="form-select" name="type"><option>A</option><option>AAAA</option><option>CNAME</option><option>MX</option><option>TXT</option></select></div><div class="col-md-2"><input class="form-control" name="name" value="@"></div><div class="col-md-5"><input class="form-control" name="value" placeholder="IP / value"></div><div class="col-md-2"><input class="form-control" name="ttl" value="3600"></div><div class="col-md-1"><button class="btn btn-primary w-100">+</button></div></form><table class="table table-dark-soft"><?php foreach($recs as $r): ?><tr><td><?= e($r['type']) ?></td><td><?= e($r['name']) ?></td><td><code><?= e($r['value']) ?></code></td><td><?= (int)$r['ttl'] ?></td><td><form method="post"><?= csrf_field() ?><input type="hidden" name="action" value="delete_dns_record"><input type="hidden" name="id" value="<?= (int)$r['id'] ?>"><button class="btn btn-sm btn-outline-danger">x</button></form></td></tr><?php endforeach; ?></table></div><?php endforeach; if(!$zones): ?><div class="panel-card empty">DNS зон пока нет</div><?php endif; ?></div></div><?php }

function view_ssl(): void { $sites=db()->query('SELECT * FROM sites ORDER BY domain')->fetchAll(); $certs=run_ctl_json(['ssl-status-json'],30); $map=[]; if(!isset($certs['_error'])) foreach($certs as $c) $map[$c['domain']]=$c; ?>
<div class="panel-card mb-4"><div class="card-title-row"><h2>SSL сертификаты</h2><form method="post"><?= csrf_field() ?><input type="hidden" name="action" value="ssl_renew_all"><button class="btn btn-primary">Проверить автопродление</button></form></div><table class="table table-dark-soft align-middle"><thead><tr><th>Сайт</th><th>DNS перед SSL</th><th>Статус</th><th></th></tr></thead><tbody><?php foreach($sites as $s): $c=$map[$s['domain']]??null; $dns=run_ctl_json(['ssl-check-json',$s['domain']],20); $dnsOk=empty($dns['_error']) && !empty($dns['points_here']); ?><tr><td><b><?= e($s['domain']) ?></b><div class="small muted">A: <?= e(implode(', ', $dns['a'] ?? [])) ?: 'нет' ?></div></td><td><?php if($dnsOk): ?><span class="badge text-bg-success">DNS OK</span><?php else: ?><span class="badge text-bg-warning">DNS не готов</span><div class="small muted">Нужна A-запись на <?= e((string)($dns['public_ip'] ?? host_name())) ?></div><?php endif; ?></td><td><?= $c?'<span class="badge text-bg-'.(($c['status']??'')==='ok'?'success':'warning').'">'.e((string)($c['days_left']??'?')).' дней</span>':'<span class="badge text-bg-secondary">нет SSL</span>' ?><div class="small muted"><?= e((string)($c['expires']??'')) ?></div></td><td><button class="btn btn-sm btn-soft" data-bs-toggle="modal" data-bs-target="#ssl<?= (int)$s['id'] ?>">Выпустить</button></td></tr><div class="modal fade" id="ssl<?= (int)$s['id'] ?>"><div class="modal-dialog"><div class="modal-content"><form method="post"><?= csrf_field() ?><input type="hidden" name="action" value="ssl_site"><input type="hidden" name="id" value="<?= (int)$s['id'] ?>"><div class="modal-header"><h5>SSL для <?= e($s['domain']) ?></h5><button class="btn-close" data-bs-dismiss="modal" type="button"></button></div><div class="modal-body"><div class="alert alert-info">Перед выпуском SSL домен должен иметь публичную A-запись на IP сервера. Сейчас панель проверяет это автоматически и не запускает Certbot, если DNS не готов.</div><input class="form-control" name="email" type="email" placeholder="email@example.com" required></div><div class="modal-footer"><button class="btn btn-primary">Выпустить</button></div></form></div></div></div><?php endforeach; ?></tbody></table></div><?php }

function view_php(): void { $sites=db()->query('SELECT * FROM sites ORDER BY domain')->fetchAll(); $versions=run_ctl_json(['php-list-json'],30); ?>
