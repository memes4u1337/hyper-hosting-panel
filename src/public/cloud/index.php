...
/**
 * HYPER CLOUD v102 — repaired archive editor + standalone HTML website preview, with upload progress and private sharing preserved.
...
function hc_html_preview_output(string $rel, string $path, string $siteRootRel = ''): never
{
    ...
    if ($size > 8 * 1024 * 1024) throw new RuntimeException('HTML-файл слишком большой для предпросмотра');
...
function hc_archive_detect_site_root(string $archivePath, string $entry): string
{
    ...
    $list=hc_archive_entries($archivePath,50000); $set=[];
...
function hc_archive_html_preview_output(string $archiveRel,string $archivePath,string $entry,string $siteRoot=''): never
{
    ...
    $data=hc_archive_entry_bytes($archivePath,$entry,8*1024*1024); $html=(string)$data['content'];
...
{
    $entry=hc_archive_entry_clean($entry); $data=hc_archive_entry_bytes($archivePath,$entry,64*1024*1024); $content=(string)$data['content']; $siteRoot=$siteRoot!==''?hc_archive_entry_clean($siteRoot):''; $base=dirname($entry); if($base==='.')$base=''; $ext=strtolower(pathinfo($entry,PATHINFO_EXTENSION));
...
function hc_archive_entries(string $path, int $limit = 1000): array
{
    ...
    foreach ($commands as $cmd) {
        ...
        if (str_contains($cmd[0], '7z')) {
            ...
            if (count($entries) < $limit && !empty($current['Path'])) {
                $n=(string)$current['Path'];
...
function hc_archive_entry_lookup(string $archivePath, string $entry): array
{
    ...
    $list = hc_archive_entries($archivePath, 50000);
    if (!empty($list['ok'])) {
...
function hc_archive_capture_command(string $command, int $limit): array
{
    $limit=max(1,$limit);
    if (!function_exists('proc_open')) {
        $tmp=tempnam(sys_get_temp_dir(),'hc-cap-');
        if ($tmp===false) return ['ok'=>false,'data'=>''];
        exec($command.' > '.escapeshellarg($tmp).' 2>/dev/null',$ignore,$code);
        $data=@file_get_contents($tmp,false,null,0,$limit+1); @unlink($tmp);
        return ['ok'=>$code===0,'data'=>is_string($data)?$data:''];
    }
    $spec=[0=>['pipe','r'],1=>['pipe','w'],2=>['file','/dev/null','a']];
    $proc=@proc_open($command,$spec,$pipes);
    if (!is_resource($proc)) return ['ok'=>false,'data'=>''];
    fclose($pipes[0]);
    $data=stream_get_contents($pipes[1],$limit+1);
    fclose($pipes[1]);
    $code=proc_close($proc);
    return ['ok'=>$code===0 || (is_string($data) && strlen($data)>$limit),'data'=>is_string($data)?$data:''];
}

function hc_archive_entry_bytes(string $archivePath, string $entry, int $limit = 67108864): array
{
    ...
    if ($knownSize>$limit) throw new RuntimeException('Файл слишком большой для просмотра');
        ...
        if ($zip->open($archivePath)===true) {
            ...
            if (is_resource($stream)) {
                ...
                while (!feof($stream) && strlen($data)<=$limit) {
                    $chunk=fread($stream,min(262144,$limit+1-strlen($data)));
                    if ($chunk===false) break;
                    $data.=$chunk;
                }
                fclose($stream); $zip->close();
                if (strlen($data)>$limit) throw new RuntimeException('Файл слишком большой для просмотра');
                return ['content'=>$data,'entry'=>$canonical,'actual'=>$actual,'size'=>strlen($data),'writable'=>hc_archive_writer($archivePath)!==''];
            }
            $idx=$found['index'];
            if (is_int($idx)) {
                $data=$zip->getFromIndex($idx,$limit+1); $zip->close();
                if (is_string($data)) {
                    if (strlen($data)>$limit) throw new RuntimeException('Файл слишком большой для просмотра');
                    return ['content'=>$data,'entry'=>$canonical,'actual'=>$actual,'size'=>strlen($data),'writable'=>hc_archive_writer($archivePath)!==''];
                }
    ...
    if (str_ends_with(strtolower($archivePath),'.zip') && is_executable('/usr/bin/unzip')) {
        $cmd='timeout 20s /usr/bin/unzip -p '.escapeshellarg($archivePath).' '.escapeshellarg($actual);
        $result=hc_archive_capture_command($cmd,$limit);
    }
    if (!$result['ok'] && is_executable('/usr/bin/7z')) {
        $cmd='timeout 20s /usr/bin/7z x -so -bd -y '.escapeshellarg($archivePath).' '.escapeshellarg($actual);
        $result=hc_archive_capture_command($cmd,$limit);
    }
    if (!$result['ok'] && is_executable('/usr/bin/7zz')) {
        $cmd='timeout 20s /usr/bin/7zz x -so -bd -y '.escapeshellarg($archivePath).' '.escapeshellarg($actual);
        $result=hc_archive_capture_command($cmd,$limit);
    }
    if (!$result['ok'] && is_executable('/usr/bin/bsdtar')) {
        $cmd='timeout 20s /usr/bin/bsdtar -xOf '.escapeshellarg($archivePath).' '.escapeshellarg($actual);
        $result=hc_archive_capture_command($cmd,$limit);
    }
...
function hc_archive_entry_write(string $archivePath, string $entry, string $content): void
{
    ...
    try {
        if ($writer==='ziparchive') {
            ...
            if (!$zip->deleteIndex($idx) || !$zip->addFromString($actual,$content)) { $zip->close(); throw new RuntimeException('Не удалось обновить файл внутри ZIP'); }
        ...
        @copy($backup,$archivePath); throw $e;
    }
...
if ($cloudAction !== '') {
    try {
        ...
        if ($cloudAction === 'html_preview') hc_html_preview_output($relAction, $actionPath, $siteRootAction);
        if ($cloudAction === 'html_asset') hc_html_asset_output($relAction, $actionPath, $siteRootAction);
        if ($cloudAction === 'archive_html_preview') hc_archive_html_preview_output($relAction, $actionPath, (string)($_GET['entry'] ?? ''), $siteRootAction);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $isAjaxUpload = $action === 'upload' && (string)($_POST['ajax_upload'] ?? '') === '1';
    $current = '';
    try {
        ...
        if (($isAjaxUpload ?? false) && ($action ?? '') === 'upload') {
...
<link rel="stylesheet" href="/cloud/cloud.css?v=102">
</head>
    ...
    <main class="hc-main" id="hcMain">
      ...
      <section class="hc-files-section">
        ...
        <div class="hc-grid" id="hcFilesGrid">
          <?php foreach($rows as $row): $kind=hc_kind($row['path']); $isDir=(bool)$row['is_dir']; $isHtmlFile=!$isDir&&hc_is_html_name($row['name']); $isEditableFile=!$isDir&&hc_is_editable_text_name($row['name'])&&(int)$row['size']<=2*1024*1024; $share=$isDir?null:($shareByPath[$row['rel']]??null); $shareToken=is_array($share)?(string)($share['token']??''):''; $shareUrl=$shareToken!==''?hc_public_share_url($shareToken):''; $previewHref=$isDir?hc_url(['path'=>$row['rel']]):hc_url(array_filter(['path'=>$view==='disk'?$rel:null,'view'=>$view!=='disk'?$view:null,'preview'=>$row['rel']],static fn($v)=>$v!==null)); $openHref=$isHtmlFile?hc_url(['cloud_action'=>'html_preview','path'=>$row['rel']]):$previewHref; $editHref=$isEditableFile?hc_url(array_filter(['path'=>$view==='disk'?$rel:null,'view'=>$view!=='disk'?$view:null,'preview'=>$row['rel'],'edit'=>'1'],static fn($v)=>$v!==null)):''; ?>
          <article class="hc-file-card" data-file-name="<?= e(hc_lower($row['name'])) ?>" data-kind="<?= e($kind) ?>">
            ...
            <div class="hc-file-actions">
              <?php if(!$isDir): ?><?php if($isEditableFile): ?><a title="Редактировать код" href="<?= e($editHref) ?>"><i class="fa-solid fa-code"></i></a><?php endif; ?><a title="Скачать" href="<?= e(hc_url(['cloud_action'=>'download','path'=>$row['rel']])) ?>"><i class="fa-solid fa-download"></i></a><?php endif; ?>
...
<?php if($previewPath): $mime=hc_mime($previewPath); $previewIsArchive=hc_is_archive($previewPath); $previewIsHtml=!$previewIsArchive && hc_is_html_name($previewPath); $previewIsText=!$previewIsArchive && hc_is_text($previewPath) && hc_is_editable_text_name(basename($previewPath)) && (filesize($previewPath)?:0)<=2*1024*1024; ?>
    ...
    <div class="hc-preview-body<?= $previewIsArchive?' is-archive':'' ?><?= ($editText || $archiveEntry!=='')?' is-editor':'' ?>">
      ...
      <?php elseif($previewIsArchive): $archive=hc_archive_entries($previewPath); $archiveWritable=hc_archive_writer($previewPath)!==''; ?>
        <?php if(!empty($archive['ok'])): ?>
          ...
          <?php foreach($archive['entries'] as $entry):
            ...
            $canEdit=$entryCanonical!==''&&hc_is_editable_text_name($entryCanonical)&&(int)($entry['size']??0)<=2*1024*1024;
            $entryIsHtml=$entryCanonical!==''&&hc_is_html_name($entryCanonical);
            $entryEditHref=$canEdit?hc_url(array_filter(['path'=>$view==='disk'?$rel:null,'view'=>$view!=='disk'?$view:null,'preview'=>$previewRel,'archive_entry'=>$entryCanonical],static fn($v)=>$v!==null)):'';
            $entrySiteHref=$entryIsHtml?hc_url(['cloud_action'=>'archive_html_preview','path'=>$previewRel,'entry'=>$entryCanonical]):'';
          ?>
          <tr><td>
      ...
      <?php elseif($previewIsHtml): ?>
        <div class="hc-html-site-preview">
          ...
          <iframe class="hc-html-frame" id="hcHtmlFrame" sandbox="allow-scripts allow-forms allow-modals allow-popups allow-downloads" referrerpolicy="no-referrer" src="<?= e(hc_url(['cloud_action'=>'html_preview','path'=>$previewRel])) ?>" title="Предпросмотр <?= e(basename($previewPath)) ?>"></iframe>
        </div>
...
<script src="/cloud/cloud.js?v=102" defer></script>
</body>
</html>
