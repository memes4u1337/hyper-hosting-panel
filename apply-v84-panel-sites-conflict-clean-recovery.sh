...
BACKUP="$BASE/backups/v84-panel-sites-conflict-clean-recovery-$STAMP"
REPORT=/root/hyper-host-v84-panel-sites-conflict-clean-recovery-report.txt
ROUTING_MAP="$BASE/data/v84-routing.tsv"
INDEX_BEFORE="/tmp/hyper-host-v84-index-before-$$.json"
INDEX_AFTER="/tmp/hyper-host-v84-index-after-$$.json"
mkdir -p "$BACKUP"
...
ensure_nginx_writable(){
  local test="/etc/nginx/hyper-host-v84-write-test-$$" runtime=/opt/hyper-host/runtime/nginx mount_script=/opt/hyper-host/bin/mount-nginx-runtime.sh
...
[[ -f /opt/hyper-host/nginx_recover_v84.py ]] && cp -a /opt/hyper-host/nginx_recover_v84.py "$BACKUP/nginx_recover_v84.py"
[[ -f /opt/hyper-host/nginx_conflict_resolver_v84.py ]] && cp -a /opt/hyper-host/nginx_conflict_resolver_v84.py "$BACKUP/nginx_conflict_resolver_v84.py"
...
rollback(){
  local code=$?
  trap - ERR
  printf '[HYPER-HOST] Ошибка. Возвращаю Nginx и CLI в состояние до v84.\n' >&2
  ...
  if [[ -f "$BACKUP/nginx_recover_v84.py" ]]; then
    cp -a "$BACKUP/nginx_recover_v84.py" /opt/hyper-host/nginx_recover_v84.py
  else
    rm -f /opt/hyper-host/nginx_recover_v84.py
  fi
  if [[ -f "$BACKUP/nginx_conflict_resolver_v84.py" ]]; then
    cp -a "$BACKUP/nginx_conflict_resolver_v84.py" /opt/hyper-host/nginx_conflict_resolver_v84.py
  else
    rm -f /opt/hyper-host/nginx_conflict_resolver_v84.py
  fi
...
log 'Удаляю все старые активные маршруты управляемых доменов и создаю один точный vhost панели и отдельный vhost каждого сайта.'
install -m 0755 "$ROOT/scripts/hhctl" /usr/local/sbin/hyper-host-ctl
install -m 0755 "$ROOT/scripts/nginx_recover_v84.py" /opt/hyper-host/nginx_recover_v84.py
install -m 0755 "$ROOT/scripts/nginx_conflict_resolver_v84.py" /opt/hyper-host/nginx_conflict_resolver_v84.py
bash -n /usr/local/sbin/hyper-host-ctl
python3 -W error -m py_compile /opt/hyper-host/nginx_recover_v84.py /opt/hyper-host/nginx_conflict_resolver_v84.py
...
python3 /opt/hyper-host/nginx_recover_v84.py \
  ...
  --map "$ROUTING_MAP" \
  --cleanup-report "$BASE/data/v84-nginx-cleanup.txt"

# Resolve any duplicate host that survived under an unexpected filename/include.
# The canonical v84 route is kept; the exact duplicate token is removed only from
# the other active config. Full /etc/nginx is already backed up above.
CONFLICT_REPAIR_LOG="$BASE/data/v84-conflict-repair.txt"
: > "$CONFLICT_REPAIR_LOG"
for round in 1 2 3 4 5; do
  ...
  for conflict_host in "${CONFLICT_HOSTS[@]}"; do
    ...
    python3 /opt/hyper-host/nginx_conflict_resolver_v84.py --host "$conflict_host" --keep "$keep_conf" | tee -a "$CONFLICT_REPAIR_LOG"
  done
done
...
probe_route(){
  ...
  probe="hyper-host-v84-${label}-${STAMP}-$$.txt"
  body="HYPER-HOST-V84-${label}-${STAMP}-$$"
...
PANEL_PROBE="hyper-host-v84-panel-${STAMP}-$$.txt"
PANEL_BODY="HYPER-HOST-V84-PANEL-${STAMP}-$$"
...
RESULTS="/tmp/hyper-host-v84-results-$$.txt"; : > "$RESULTS"
...
{
  echo 'HYPER-HOST v84 — panel conflict cleanup + all sites + beta recovery'
  echo
