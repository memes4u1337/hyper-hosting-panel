...
PATCH_BACKUP_DIR="$BASE_DIR/backups/v59-nginx-logdir-$(date +%Y%m%d-%H%M%S)"
REPORT="/root/hyper-host-v59-access.txt"
...
log 'Устанавливаю только исправление каталогов логов Nginx. FTP, SQL, боты, база панели и файлы сайтов не изменяются.'
install -m 0755 "$ROOT_DIR/scripts/hhctl" "$CONTROL_BIN"

# Убираем только временные тестовые конфиги прошлых патчей. Пользовательские сайты не трогаем.
find /opt/hyper-host/runtime/nginx/sites-enabled -maxdepth 1 -type l \
  -name 'hyper-host-site-v5*-nginx-test-*.local.conf' -delete 2>/dev/null || true
find /opt/hyper-host/runtime/nginx/sites-available -maxdepth 1 -type f \
  -name 'hyper-host-site-v5*-nginx-test-*.local.conf' -delete 2>/dev/null || true
...
PROBE="/etc/nginx/.hyper-host-v59-probe-$$"
...
TEST_DOMAIN="v59-nginx-test-$(date +%s).local"
cleanup_test(){
...
[[ -f "$SITES_DIR/$TEST_DOMAIN/logs/access.log" ]] || fail 'Не создан access.log тестового сайта'
[[ -f "$SITES_DIR/$TEST_DOMAIN/logs/error.log" ]] || fail 'Не создан error.log тестового сайта'
