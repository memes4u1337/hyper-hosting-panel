...
def discover_canonical_sites(sites_root: Path, panel_domain: str) -> tuple[list[str], str]:
    """Return real site folders, excluding the domain reserved for the panel.

    The excluded folder is not renamed or deleted. This prevents a historical
    /var/www/hyper-host-sites/<panel-domain>/public_html directory from stealing
    panel traffic while preserving all files on disk.
    """
    canonical: list[str] = []
    ignored_panel_site_folder = ""
    if sites_root.is_dir():
        for folder in sorted(sites_root.iterdir()):
            domain = folder.name.lower().rstrip(".")
            if not (folder.is_dir() and (folder / "public_html").is_dir() and valid_domain(domain)):
                continue
            if domain == panel_domain:
                ignored_panel_site_folder = str(folder / "public_html")
                continue
            canonical.append(domain)
    return canonical, ignored_panel_site_folder
...
def strip_managed_names_from_file(path: Path, managed_hosts: set[str]) -> bool:
    """Remove only v85-owned host tokens from an active non-sites config.
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        if not kept:
            ...
            kept = [f"v85-disabled-{safe}-{counter}.invalid"]
...
def main() -> int:
    ...
    parser.add_argument("--map", default="/opt/hyper-host/data/v85-routing.tsv")
    parser.add_argument("--cleanup-report", default="/opt/hyper-host/data/v85-nginx-cleanup.txt")
    ...
    canonical, ignored_panel_site_folder = discover_canonical_sites(sites_root, panel_domain)
    canonical_set = set(canonical)
    if beta_domain == panel_domain:
        raise SystemExit(f"beta domain {beta_domain} is reserved for the panel")
    ...
    # filename. This keeps the v83 conflict fix: panel.hyper-host.pw could remain in an old
...
""",
    ...
    Path(args.cleanup_report).write_text(
        ...
        + "\n\nReserved panel-domain site folder (kept on disk, not served as a site):\n"
        + (ignored_panel_site_folder if ignored_panel_site_folder else "none")
        + "\n",
        encoding="utf-8",
    )
    print(
        ...
        f"panel_site_folder_excluded={ignored_panel_site_folder or 'none'}"
    )
    return 0
