...
def main() -> int:
    ...
    parser.add_argument("--map", default="/opt/hyper-host/data/v83-routing.tsv")
    ...
    # Canonical sites come from real public_html folders. No invented sites or www aliases.
    canonical: list[str] = []
    ...
    # Capture old names/sockets before replacing generated configs.
    old_names: dict[str, list[str]] = {}
    old_sock: dict[str, str] = {}
    for domain in canonical:
        conf = available / f"hyper-host-site-{domain}.conf"
        old_names[domain] = server_names_from_conf(conf)
        old_sock[domain] = php_socket_from_conf(conf)

    # Remove only generated HYPER-HOST links that caused cross-routing.
    generated_patterns = (
    ...
    for pattern in generated_patterns:
        for path in enabled.glob(pattern):
            try:
                path.unlink()
    ...
    # Remove obsolete managed files; individual per-site files are rebuilt below.
    for name in ("20-hyper-host-sites-managed.conf", "hyper-host-sites-managed.conf"):
        (available / name).unlink(missing_ok=True)

    # Neutral catch-all. It never serves panel or site files.
...
""",
    ...
    # Exact panel route. It is deliberately not a default server.
    ...
    # Canonical names always own themselves.
    for domain in canonical:
        host_owner[domain] = domain
    ...
    for domain in canonical:
        ...
        # beta is intentionally exact: no automatic www or stale aliases.
    ...
    print(f"panel={panel_domain} panel_names={','.join(panel_names)} sites={len(site_plans)} hosts={len(routing_rows)}")
    return 0
