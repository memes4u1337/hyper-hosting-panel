...
def remove_panel_site_from_db(path: Path, panel_domain: str) -> None:
    if not path.is_file():
        return
    con: sqlite3.Connection | None = None
    try:
        ...
        if "aliases" in columns:
            for rowid, raw in con.execute("SELECT rowid, aliases FROM sites").fetchall():
...
def panel_block(names: list[str], root: str, socket: str, nginx_dir: Path, ssl_pair: tuple[str, str] | None = None) -> str:
    ...
    location = /__hyper_host_v88_route__ {{ default_type text/plain; return 200 "PANEL_V88"; }}
...
def site_block(names: list[str], domain: str, root: str, socket: str, logs: str, nginx_dir: Path, ssl_pair: tuple[str, str] | None = None) -> str:
    ssl = ssl_pair is not None
    listen = https_listen() if ssl else http_listen()
    cert = ""
    ...
    location = /__hyper_host_v88_route__ {{ default_type text/plain; return 200 "SITE_V88:{q(domain)}"; }}
    location ^~ /.well-known/acme-challenge/ {{
        root /opt/hyper-host/acme-webroot;
        default_type text/plain;
        try_files $uri =404;
...
def default_block(root: str, cert: str, key: str) -> str:
    ...
    root {root};
    index index.html;
    location = /__hyper_host_v88_route__ {{ default_type text/plain; return 200 "DEFAULT_V88"; }}
...
def main() -> int:
    ...
    if beta_domain not in canonical:
        canonical.append(beta_domain)
        canonical.sort()

    remove_panel_site_from_db(db_path, panel_domain)
    ...
    for domain in canonical:
        ...
        candidates = split_names(row.get("aliases", "")) + old_aliases.get(domain, [])
        aliases: list[str] = []
    ...
    panel_text = panel_block(panel_names, str(panel_root), args.panel_php_sock, nginx_dir)
    if panel_pair:
        panel_text += "\n" + panel_block([panel_domain], str(panel_root), args.panel_php_sock, nginx_dir, panel_pair)
    ...
    for plan in plans:
        ...
        text = site_block(names, domain, root, socket, logs, nginx_dir)
        ...
        for pair, ssl_names in groups.items():
            text += "\n" + site_block(ssl_names, domain, root, socket, logs, nginx_dir, pair)
