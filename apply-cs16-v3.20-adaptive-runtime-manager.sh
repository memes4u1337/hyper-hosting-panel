...
# HYPER-HOST CS16 v3.20 — ADAPTIVE RUNTIME MANAGER
#
...
#
# Usage:
#   bash apply-cs16-v3.20-adaptive-runtime-manager.sh 15
...
BACKUP="/root/hyper-cs16-v3.20-backup-${STAMP}"
LOG="/root/hyper-cs16-v3.20-${STAMP}.log"
...
echo "============================================================"
echo " HYPER-HOST CS16 ADAPTIVE RUNTIME MANAGER v3.20"
echo "============================================================"
echo "Server:     $SID"
...
PATCHER="$(mktemp /tmp/hh-v320.XXXXXX.py)"
ROOTS="$(mktemp /tmp/hh-v320-roots.XXXXXX)"
NGTMP="$(mktemp /tmp/hh-v320-nginx.XXXXXX)"
...
CTL_MARKER = '# >>> HYPER-HOST v3.20 ADAPTIVE RUNTIME >>>'

HELPER = r'''
# >>> HYPER-HOST v3.20 ADAPTIVE RUNTIME >>>
def _v320_strings(path:Path)->str:
    if not path.is_file():
        return ''
...
def _v320_version_from_strings(path:Path,kind:str)->str:
    text=_v320_strings(path)
    patterns={
        'amxx':[
...
def _v320_runtime_policy(c:dict)->dict:
...
def _v320_find_metamod(cstrike:Path):
...
def _v320_find_amxx(cstrike:Path):
    return _first_existing(cstrike,[
...
def _v320_sha256(path:Path)->str:
    try:
        h=hashlib.sha256()
...
def _v320_same_binary(a:Path,b:Path)->bool:
    if not (a and b and a.is_file() and b.is_file()):
        return False
    try:
        ...
        return False
    ha=_v320_sha256(a); hb=_v320_sha256(b)
    return bool(ha and hb and ha==hb)


def _v320_install_clean_gamedll(path:Path)->str:
    # A cs.so that is actually Metamod causes recursive Metamod -> AMXX loading.
    # ReHLDS gets ReGameDLL. Classic HLDS gets the clean base CS GameDLL.
    rehlds=bool(
        _binary_contains(path/'engine_i486.so',b'rehlds') or
        _binary_contains(path/'hlds_linux',b'rehlds')
    )
    target=path/'cstrike/dlls/cs.so'
    target.parent.mkdir(parents=True,exist_ok=True)

    if rehlds:
        with tempfile.TemporaryDirectory(prefix='hhcs16-v320-regamedll-') as td:
            td=Path(td); z=td/'regamedll.zip'
            download(REGAMEDLL_URL,z); extract_zip(z,td/'regamedll')
            candidates=list((td/'regamedll').rglob('cstrike/dlls/cs.so'))
            if not candidates:
                raise RuntimeError('ReGameDLL cs.so was not found in the managed archive')
            shutil.copy2(candidates[0],target)
        return 'replaced recursive/invalid cs.so with managed ReGameDLL'

    rt=load_runtime()
    base=Path(rt.get('base_game_dir','/srv/hyper-cs16/base-hlds'))
    ensure_base('classic')
    source=base/'cstrike/dlls/cs.so'
    if not source.is_file():
        raise RuntimeError('Clean classic cstrike/dlls/cs.so is missing from base runtime')
    shutil.copy2(source,target)
    return 'replaced recursive/invalid cs.so with clean classic CS GameDLL'


def _v320_binary_chain_repairs(path:Path)->list[str]:
    cstrike=path/'cstrike'
    actions=[]
    meta=_v320_find_metamod(cstrike)
    amxx=_v320_find_amxx(cstrike)
    game=cstrike/'dlls/cs.so'

    # The most important recursion case: Metamod loads cs.so, but cs.so itself
    # is another copy of Metamod. That produces duplicate meta/amxx commands and
    # eventually "same handler can't be used twice on the hookchain".
    if meta and game.is_file():
        game_strings=_v320_strings(game).lower()
        recursive=(
            _v320_same_binary(meta,game) or
            'metamod-r version' in game_strings or
            ('metamod version' in game_strings and 'counter-strike' not in game_strings)
        )
        if recursive:
            actions.append(_v320_install_clean_gamedll(path))

    # Do not let AMXX load Metamod or its own loader as an AMXX module.
    modules_dir=cstrike/'addons/amxmodx/modules'
    if modules_dir.is_dir():
        for so in modules_dir.glob('*.so'):
            if not so.is_file():
                continue
            duplicate=False
            if meta and _v320_same_binary(so,meta):
                duplicate=True
            if amxx and _v320_same_binary(so,amxx):
                duplicate=True
            if duplicate:
                disabled=so.with_name(so.name+'.disabled-v320')
                try:
                    if disabled.exists(): disabled.unlink()
                    os.replace(so,disabled)
                    actions.append('disabled recursive AMXX module binary '+so.name)
                except Exception:
                    pass
...
def _v320_ensure_reapi_policy(stage:Path,c:dict)->dict:
    policy=_v320_runtime_policy(c)
    req=_build_requires_reapi(stage/'cstrike')
    if not req.get('required'):
        return {'required':False,'actions':[],**req}

    # Explicit "assembly" means do not replace Engine/GameDLL behind the user's back.
    if policy.get('engine')=='assembly':
        has_rehlds=_binary_contains(stage/'engine_i486.so',b'rehlds') or _binary_contains(stage/'hlds_linux',b'rehlds')
        has_regame=_binary_contains(stage/'cstrike/dlls/cs.so',b'regamedll')
        return {
            'required':True,
            'actions':[],
            'rehlds':has_rehlds,
            'regamedll':has_regame,
            'policy_blocked':not (has_rehlds and has_regame),
            **req,
        }

    return _ensure_reapi_compatible_stack(stage)


def _v320_runtime_inventory(path:Path)->dict:
    cstrike=path/'cstrike'
    ...
    meta_ver=_v320_version_from_strings(meta,'metamod') if meta else ''
    amxx_ver=_v320_version_from_strings(amxx,'amxx') if amxx else ''
    reapi_ver=_v320_version_from_strings(reapi,'reapi') if reapi.is_file() else ''
    ...
    return {
        ...
        'metamod':{
            'kind':'Metamod-R' if (meta and ('metamod-r' in _v320_strings(meta).lower())) else ('Metamod' if meta else 'missing'),
            'version':meta_ver,
...
def _v320_sanitize_loader_chain(path:Path)->dict:
    cstrike=path/'cstrike'
    actions=[]

    # Repair binary recursion before writing loader files.
    actions.extend(_v320_binary_chain_repairs(path))

    meta=_v320_find_metamod(cstrike)
    amxx=_v320_find_amxx(cstrike)

    # Exactly one Linux GameDLL directive.
    ...
    # Metamod should auto-load cstrike/dlls/cs.so. Any gamedll override from
    # foreign hosting exports can recurse into Metamod or load a second proxy.
    meta_cfg=cstrike/'addons/metamod/config.ini'
    if meta_cfg.is_file():
        ...
        for raw in text.replace('\r\n','\n').replace('\r','\n').splitlines():
            ...
            if st and not st.startswith((';','#','//')) and re.match(r'(?i)^gamedll\b',st):
                out.append('; HYPER-HOST v3.20 disabled GameDLL override: '+raw)
                changed=True
            else:
                out.append(raw)
        if changed:
    ...
    # Exactly one AMXX loader. Never let plugins.ini load Metamod itself.
    ...
    for raw in text.replace('\\r\\n','\n').replace('\\n','\n').replace('\\r','\n').replace('\r\n','\n').replace('\r','\n').splitlines():
        ...
        if active and 'addons/metamod' in low and '.so' in low:
            out.append('; HYPER-HOST v3.20 disabled recursive Metamod self-load: '+raw)
            continue

        if active and ('amxmodx' in low or 'amxmodx_mm' in low):
    ...
    # De-duplicate AMXX modules and disable obvious loader recursion.
    ...
    if modules.is_file():
        ...
        for raw in text.replace('\\r\\n','\n').replace('\\n','\n').replace('\\r','\n').replace('\r\n','\n').replace('\r','\n').splitlines():
            ...
            if 'metamod' in token or 'amxmodx_mm' in token:
                out.append('; HYPER-HOST v3.20 disabled recursive loader-as-module: '+raw)
                changed=True
                continue

            if token and token in seen:
                out.append('; HYPER-HOST v3.20 duplicate disabled: '+raw)
                changed=True
                continue
        ...
        if changed:
            modules.write_text('\n'.join(out).rstrip()+'\n',encoding='latin1')
            actions.append('sanitized modules.ini')

    # Re-check binary identity after text-chain cleanup.
    actions.extend(_v320_binary_chain_repairs(path))

    return {'ok':True,'actions':list(dict.fromkeys(actions)),'inventory':_v320_runtime_inventory(path)}


def _v320_prepare_runtime(path:Path,c:dict)->dict:
    policy=_v320_runtime_policy(c)
    before=_v320_runtime_inventory(path)
    actions=[]

    # AUTO and ASSEMBLY preserve uploaded versions on first pass.
    # MANAGED replaces only the selected component family.
    if policy['engine']=='managed':
        install_rehlds(path)
        actions.append('managed ReHLDS/ReGameDLL selected')

    if policy['metamod']=='managed':
        current=_v320_runtime_inventory(path)
        if str((current.get('engine') or {}).get('kind') or '')=='ReHLDS':
            ...
            actions.append('managed Metamod selected for classic HLDS')
    ...
    clean=_v320_sanitize_loader_chain(path)
    actions.extend(clean.get('actions') or [])
    after=_v320_runtime_inventory(path)
...
def _v320_adaptive_repair(path:Path,c:dict,journal:str)->dict:
    policy=_v320_runtime_policy(c)
    ...
    duplicate_meta=any(x in low for x in (
        ...
        'already loaded (status=running)',
    ))
    bad_amxx=any(x in low for x in (
        'failed query plugin',
        'failed to load plugin \'amxmodx',
        'amxmodx_mm_i386.so\\n',
        'zombie plague core is configured but not running',
        'amxx loaded only 0 of',
    ))
    ...
    if duplicate_meta:
        r=_v320_sanitize_loader_chain(path)
    ...
    if needs_reapi and policy['engine']=='auto':
        install_rehlds(path)
        _v320_sanitize_loader_chain(path)
...
def runtime_repair_chain(sid:int):
    ...
    repair=_v320_sanitize_loader_chain(path)
    ...
    return {'ok':ok,'id':sid,'backup':str(backup),'query':detail,'actions':repair.get('actions') or [],'runtime':_v320_runtime_inventory(path)}


def runtime_apply(sid:int):
    require_root(); c=load_server(sid); path=safe_server_path(sid)
    policy=_v320_runtime_policy(c); actions=[]
    ...
    actions.extend((_v320_sanitize_loader_chain(path).get('actions') or []))
    normalize_permissions(path)
    run(['systemctl','reset-failed',f'hyper-cs16@{sid}.service'],check=False)
    run(['systemctl','start',f'hyper-cs16@{sid}.service'],check=False,timeout=60)
    ok,detail=wait_server_ready(load_server(sid),45.0)
    return {'ok':ok,'id':sid,'backup':str(backup),'query':detail,'actions':actions,'policy':policy,'runtime':_v320_runtime_inventory(path)}
# <<< HYPER-HOST v3.20 ADAPTIVE RUNTIME <<<
'''
...
def patch_controller(path:Path):
    ...
    if 'def _v316_refresh_amxx_platform(' not in src:
        raise RuntimeError('v3.20 expects the current v3.16/v3.18 runtime framework')

    if CTL_MARKER not in src:
        pos=src.find('\ndef install_custom_build(')
        if pos<0:
            raise RuntimeError('install_custom_build not found')
        src=src[:pos]+'\n'+HELPER+src[pos:]

    # Work on install_custom_build semantically. Do not require one exact
    # whitespace/comment block from a previous patch version.
    st=src.find('def install_custom_build(')
    en=src.find('\ndef ',st+10)
    if st<0:
        raise RuntimeError('install_custom_build not found after helper insertion')
    if en<0:
        en=len(src)
    fn=src[st:en]

    # Replace any forced universal runtime generation from v3.13-v3.18.
    fn=re.sub(
        r"(?m)^        platform_runtime=_v(?:313|314|316)_normalize_platform(?:_runtime)?\(stage\)\s*$",
        "        platform_runtime=_v320_prepare_runtime(stage,c)",
        fn,
        count=1,
    )

    if 'platform_runtime=_v320_prepare_runtime(stage,c)' not in fn:
        # If the call was somehow absent, insert it before ReAPI/preflight.
        anchors=[
        ...
        anchor=next((a for a in anchors if a in fn),None)
        if anchor is None:
            raise RuntimeError('no safe adaptive runtime insertion point found')
        block=(
            '        # HYPER-HOST v3.20 adaptive runtime selection\n'
            '        platform_runtime=_v320_prepare_runtime(stage,c)\n'
            "        runtime_profile=_analyze_build_runtime(stage/'cstrike')\n"
        )
        fn=fn.replace(anchor,block+anchor,1)

    # Remove the unconditional "profile=rehlds" introduced by the old universal
    # importer, but keep the later conditional profile update after ReAPI repair.
    normpos=fn.find('platform_runtime=_v320_prepare_runtime(stage,c)')
    reapipos=fn.find('reapi_compat=')
    if normpos>=0 and reapipos>normpos:
        middle=fn[normpos:reapipos]
        middle=middle.replace("        c['profile']='rehlds'\n",'',1)
        fn=fn[:normpos]+middle+fn[reapipos:]

    # ReAPI obeys runtime policy now.
    fn=fn.replace(
        'reapi_compat=_ensure_reapi_compatible_stack(stage)',
        'reapi_compat=_v320_ensure_reapi_policy(stage,c)',
        1,
    )

    # Ensure chain is sanitized after ReAPI may have changed Engine/GameDLL.
    reapi_line='        reapi_compat=_v320_ensure_reapi_policy(stage,c)\n'
    if reapi_line in fn:
        pos=fn.find(reapi_line)
        tail=fn[pos:pos+500]
        if '_v320_sanitize_loader_chain(stage)' not in tail:
            fn=fn.replace(
                reapi_line,
                reapi_line+
                '        _v320_sanitize_loader_chain(stage)\n'
                "        runtime_profile=_analyze_build_runtime(stage/'cstrike')\n",
                1,
            )

    # Replace any old runtime suffix. If none exists, add after authoritative
    # import_mode assignment.
    suffix=(
        "        import_mode=import_mode+'+runtime-v320-'+"
        "str(platform_runtime.get('policy',{}).get('engine','auto'))+'-'+"
        "str(platform_runtime.get('policy',{}).get('metamod','auto'))+'-'+"
        "str(platform_runtime.get('policy',{}).get('amxx','auto'))"
    )

    fn=re.sub(
        r"(?m)^        import_mode=import_mode\+'+(?:managed-runtime-v\d+|runtime-v\d+-.*)'\s*$",
        suffix,
        fn,
    )

    if '+runtime-v320-' not in fn:
        assign_re=re.compile(
            r"(?m)^        import_mode='exact-server-root' if layout=='server-root' else 'exact-cstrike-archive'\s*$"
        )
        if not assign_re.search(fn):
            raise RuntimeError('authoritative import_mode assignment not found')
        fn=assign_re.sub(lambda m: m.group(0)+'\n'+suffix,fn,count=1)

    src=src[:st]+fn+src[en:]

    # Add adaptive retry immediately before the final hard-failure journal block.
    st=src.find('def install_custom_build(')
    en=src.find('\ndef ',st+10)
    if en<0: en=len(src)
    fn=src[st:en]

    if '_v320_adaptive_repair(path,c,journal)' not in fn:
        pattern=(
            "            if not ok:\n"
        ...
        adaptive=(
            ...
            "                    adaptive=_v320_adaptive_repair(path,c,journal)\n"
            ...
            "                    platform_runtime['active']=_v320_runtime_inventory(path)\n"
            "                if not ok:\n"
            "                    journal=_recent_failure_log(sid,220)\n"
        )
        if pattern not in fn:
            raise RuntimeError('final hard-failure journal block not found')
        fn=fn.replace(pattern,adaptive,1)
        src=src[:st]+fn+src[en:]

    # Enrich success payload when the stable v3.16 field exists.
    ...
    if old_result in src:
        src=src.replace(
            old_result,
            "'runtime_fill':runtime_fill,'platform_runtime':platform_runtime,"
            "'assembly_runtime':platform_runtime.get('assembly',{}),"
            "'active_runtime':_v320_runtime_inventory(path),"
            "'runtime_policy':_v320_runtime_policy(c),"
            "'reapi_compat':reapi_compat,'archive_manifest':manifest,",
            1
        )
    ...
    if "sp.add_parser('runtime-policy-set')" not in src:
        ...
        src=src.replace(
            parser_anchor,
            parser_anchor+"    q=sp.add_parser('runtime-policy-set'); q.add_argument('id',type=int)\n",
            1
        )
    ...
    if "elif args.cmd=='runtime-status'" not in src:
        if branch_anchor not in src:
            raise RuntimeError('CLI branch anchor missing')
        src=src.replace(
            branch_anchor,
            branch_anchor+
            "        elif args.cmd=='runtime-status': result=runtime_status(args.id)\n"
    ...
    if missing:
        raise RuntimeError('controller verification failed: '+repr(missing))

    tmp=path.with_name(path.name+'.v320tmp')
...
echo
echo "[2/9] Patching LIVE controller with adaptive runtime logic (semantic patch; old whitespace does not matter)..."
...
if [[ -f "/var/lib/hyper-cs16/servers/$SID.json" ]]; then
  ...
  if [[ "$REPAIR_RC" -ne 0 ]]; then
    ...
    echo "[WARN] v3.20 itself IS installed; the next assembly upload will use adaptive runtime logic."
  else
...
echo
echo "============================================================"
echo " v3.20 INSTALLED SUCCESSFULLY"
echo "============================================================"
