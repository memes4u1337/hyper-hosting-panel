...
[[ -f "$SRC/bin/hyper-cs16-ctl" && -f "$SRC/bin/hyper-cs16-monitor" && -f "$SRC/public/index.php" && -f "$SRC/app/bootstrap.php" ]] || {
  echo '[ERROR] Run this script from the v3.4-exactbuild patched HYPER-HOST repository root' >&2; exit 2;
}
BASE='/opt/hyper-cs16'
DOMAIN='www.avito.hyper-host.pw'
CANONICAL_PUBLIC='/var/www/hyper-host-sites/www.avito.hyper-host.pw/public_html'

say(){ printf '\033[1;36m[CS16 v3.4-exactbuild]\033[0m %s\n' "$*"; }
warn(){ printf '\033[1;33m[CS16 v3.4-exactbuild WARNING]\033[0m %s\n' "$*" >&2; }
fail(){ printf '\033[1;31m[CS16 v3.4-exactbuild ERROR]\033[0m %s\n' "$*" >&2; exit 1; }

say 'Installing dependencies...'
export DEBIAN_FRONTEND=noninteractive
apt-get update -qq
apt-get install -y --no-install-recommends rsync curl p7zip-full python3-pymysql mariadb-client libc6-i386 lib32gcc-s1 lib32stdc++6 >/dev/null
...
say 'No bundled game/server build files are included. Exact Linux cstrike runtimes + required ReHLDS outer engine compatibility are preserved from user uploads.'
...
say "Deploying v3.4-exactbuild to ${#ROOTS[@]} detected document root(s)..."
for DOCROOT in "${!ROOTS[@]}"; do
  ...
  grep -q "HYPER_CS16_PANEL_BUILD = '3.4-exactbuild'" "$APPDIR/bootstrap.php" || fail "Wrong bootstrap deployed to $APPDIR/bootstrap.php"
...
say 'Verifying that nginx serves v3.4-exactbuild, not cached/old PHP...'
...
if [[ -n "$LIVE" ]]; then
  if grep -q 'style.css?v=340' <<<"$LIVE"; then
    say 'Live web verification OK: panel v3.4-exactbuild is being served.'
  else
    warn 'Live HTTPS answered, but v3.4-exactbuild marker was not found. Check nginx root/server_name; file deployment itself passed.'
  fi
else
...
echo
say 'DONE. Exact full-build import + ReHLDS compatibility + runtime module/plugin validation + FastDL + SQL/statistics are installed. No game/server build files are bundled.'
say 'Open the panel and press Ctrl+F5. The sidebar must show Panel v3.4-exactbuild.'
