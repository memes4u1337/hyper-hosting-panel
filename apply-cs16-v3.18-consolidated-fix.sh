...
# HYPER-HOST CS16 v3.18 — CONSOLIDATED IMPORT + AMXX + MAP FIX
#
...
BACKUP="/root/hyper-cs16-v3.18-backup-${STAMP}"
LOG="/root/hyper-cs16-v3.18-${STAMP}.log"
MANIFEST="$BACKUP/restore-paths.txt"
PATCHER="$(mktemp /tmp/hh-v318.XXXXXX.py)"
ROOT_FILE="$(mktemp /tmp/hh-v318-roots.XXXXXX)"
NGTMP="$(mktemp /tmp/hh-v318-nginx.XXXXXX)"
...
cat <<EOF
============================================================
 HYPER-HOST CS16 CONSOLIDATED FIX v3.18
============================================================
Server:     $SID
Controller: $LIVE_CTL
Repo:       $REPO
...
echo
echo "[POST-FIX] Repairing CURRENT server #$SID AMXX loader / text files..."

STATE="/var/lib/hyper-cs16/servers/$SID.json"
if [[ -f "$STATE" ]]; then
  readarray -t _V318_INFO < <(python3 - "$STATE" <<'PY_V318_STATE'
import json,sys
from pathlib import Path
...
PY_V318_STATE
  ...
  V318_SERVER_PATH="${_V318_INFO[0]}"
  V318_PORT="${_V318_INFO[1]}"
  V318_META="$V318_SERVER_PATH/cstrike/addons/metamod/plugins.ini"
  V318_LOADER="$V318_SERVER_PATH/cstrike/addons/amxmodx/dlls/amxmodx_mm_i386.so"
  V318_MAPCYCLE="$V318_SERVER_PATH/cstrike/mapcycle.txt"
  V318_MAPSINI="$V318_SERVER_PATH/cstrike/addons/amxmodx/configs/maps.ini"

  backup_one "$V318_META"
  backup_one "$V318_MAPCYCLE"
  backup_one "$V318_MAPSINI"

  python3 - "$V318_META" "$V318_MAPCYCLE" "$V318_MAPSINI" <<'PY_V318_REPAIR'
from pathlib import Path
import sys
...
def split_accidental(text:str):
    # v3.16 accidentally wrote backslash+n literally.
    return (
        text
        .replace('\\r\\n','\n')
        .replace('\\n','\n')
...
if meta.exists():
    text=split_accidental(meta.read_bytes().decode('latin1'))
    out=[]
    seen=False
...
for p in (mapcycle,mapsini):
    if not p.exists():
        continue
    text=split_accidental(p.read_bytes().decode('utf-8','replace'))
    out=[]
    seen=set()
    for raw in text.splitlines():
        name=raw.strip()
...
PY_V318_REPAIR
  ...
  [[ -f "$V318_LOADER" ]] || {
    echo "[ERROR] Managed AMXX loader missing: $V318_LOADER"
    false
  }

  python3 - "$V318_META" <<'PY_V318_META_VERIFY'
from pathlib import Path
import sys
p=Path(sys.argv[1])
raw=p.read_bytes()
...
amxx=[x for x in active if 'amxmodx' in x.lower()]
expected=['linux addons/amxmodx/dlls/amxmodx_mm_i386.so']
if amxx != expected:
    raise SystemExit('[ERROR] invalid AMXX loader lines: '+repr(amxx))
print('[OK] current plugins.ini has a canonical AMXX loader with real newline')
PY_V318_META_VERIFY

  echo "[POST-FIX] Restarting $SID once..."
  ...
  for _ in $(seq 1 45); do
    if systemctl is-active --quiet "hyper-cs16@$SID.service"; then
      if [[ "$V318_PORT" -gt 0 ]] && ss -lun 2>/dev/null | awk '{print $5}' | grep -Eq "[:.]${V318_PORT}$"; then
        V318_OPENED=1
        break
      fi
    fi
  ...
  if [[ "$V318_OPENED" -eq 1 ]]; then
    sleep 4
    echo "--- meta list ---"
    V318_META_LIST="$("$LIVE_CTL" rcon "$SID" "meta list" 2>&1 || true)"
    echo "$V318_META_LIST"
    echo "--- amxx plugins ---"
    V318_AMXX_LIST="$("$LIVE_CTL" rcon "$SID" "amxx plugins" 2>&1 || true)"
    echo "$V318_AMXX_LIST"

    python3 - "$V318_META_LIST" "$V318_AMXX_LIST" <<'PY_V318_RCON_VERIFY'
import json,re,sys
...
meta=unwrap(sys.argv[1])
amxx=unwrap(sys.argv[2])

if 'amxmodx_mm_i386.so\\n' in meta:
    raise SystemExit('[ERROR] Metamod still sees literal \\n in AMXX filename')
if 'badf load' in meta.lower():
    raise SystemExit('[ERROR] Metamod still reports badf loader:\n'+meta)
if not any(x in meta.lower() for x in ('amx mod x','amxmodx')):
    raise SystemExit('[ERROR] AMXX is not visible in meta list:\n'+meta)

if not amxx.strip() or 'unknown command' in amxx.lower():
    raise SystemExit('[ERROR] amxx plugins is not operational:\n'+amxx)
if re.search(r'\b0\s+plugins?\b',amxx,re.I):
    raise SystemExit('[ERROR] AMXX still reports zero plugins:\n'+amxx)

print('[OK] Metamod AMXX loader is clean')
print('[OK] AMXX command interface is operational')
PY_V318_RCON_VERIFY
  else
    echo "[WARN] Server did not expose UDP within 45 seconds."
    journalctl -u "hyper-cs16@$SID.service" -n 100 --no-pager || true
    false
  fi
else
  echo "[INFO] Server #$SID does not exist; current-server repair skipped."
fi

echo
echo "[FINAL VERIFY] Checking generated controller source..."
python3 - "$LIVE_CTL" "$REPO/cs16-panel/bin/hyper-cs16-ctl" <<'PY_V318_SOURCE_VERIFY'
from pathlib import Path
import sys

for name in sys.argv[1:]:
    s=Path(name).read_text(encoding='utf-8',errors='surrogateescape')
    checks={
        'runtime helper': 'def _v316_refresh_amxx_platform(' in s,
        'real AMXX newline': "mp.write_text('\\n'.join(output).rstrip()+'\\n',encoding='latin1')" in s,
        'no literal AMXX newline generator': "mp.write_text('\\\\n'.join(output).rstrip()+'\\\\n',encoding='latin1')" not in s,
        'real map newline': "body='\\n'.join(allowed).rstrip()+'\\n'" in s,
        'no literal map newline generator': "body='\\\\n'.join(allowed).rstrip()+'\\\\n'" not in s,
        'map regex': '[A-Za-z0-9_-]{1,64}' in s,
        'no doubled regex': '[A-Za-z0-9_-]{{1,64}}' not in s,
    }
    bad=[k for k,v in checks.items() if not v]
    if bad:
        raise SystemExit(f'[ERROR] {name}: '+', '.join(bad))
    print('[OK]',name,'newline/map/runtime source verified')
PY_V318_SOURCE_VERIFY
...
echo
echo "============================================================"
echo " v3.18 INSTALLED SUCCESSFULLY"
echo "============================================================"
echo "FIXED: import_mode is no longer read before assignment."
echo "FIXED: clean GitHub repo + already-patched live controller are both supported."
echo "FIXED: future uploaded builds normalize runtime before first start with real newlines."
