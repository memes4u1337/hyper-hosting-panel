...
[[ -f "$SRC/bin/hyper-cs16-ctl" && -f "$SRC/bin/hyper-cs16-monitor" && -f "$SRC/bin/hyper-cs16-migrate-v27" && -f "$SRC/bin/hyper-cs16-migrate-v28" && -f "$SRC/public/index.php" ]] || {
  echo '[ERROR] Run this script from the v2.8 patched HYPER-HOST repository root' >&2; exit 2;
}
...
say(){ printf '\033[1;36m[CS16 v2.8]\033[0m %s\n' "$*"; }
warn(){ printf '\033[1;33m[CS16 v2.8 WARNING]\033[0m %s\n' "$*" >&2; }
fail(){ printf '\033[1;31m[CS16 v2.8 ERROR]\033[0m %s\n' "$*" >&2; exit 1; }
...
# v2.8: STATE_ROOT must be traversable by www-data, otherwise PHP cannot even
# stat /var/lib/hyper-cs16/uploads and reports "Upload staging not configured".
...
say 'Repairing and verifying upload staging as www-data...'
STAGING_JSON="$(/usr/local/sbin/hyper-cs16-ctl staging-prepare 2>&1)" || fail "Upload staging self-test failed: $STAGING_JSON"
runuser -u www-data -- test -w /var/lib/hyper-cs16/uploads || fail 'www-data cannot write /var/lib/hyper-cs16/uploads after repair'
...
for sid in ids:
    ...
    print(f'[CS16 v2.8] #{sid}: unlimited' if cp.returncode==0 else f'[CS16 v2.8] #{sid}: unlimited warning: {(cp.stdout or "")[-500:]}')
PY
...
echo
say 'DONE.'
say 'Fixed: upload staging self-heal, exact upload/controller errors, filesystem-only AMXX list, true clean reinstall, ZIP/RAR installer rollback.'
say 'Open www.avito.hyper-host.pw, Ctrl+F5. Build upload now self-repairs staging; AMXX shows only files physically present on that server.'
