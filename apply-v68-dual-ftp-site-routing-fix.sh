...
TEST_SITE="v68-site-test-$(date +%s).test"
TARGET_PROBE="hyper-host-v68-route-$(date +%s)-$$.txt"
TARGET_PROBE_PATH=""
BACKUP_DIR="$BASE_DIR/backups/v68-dual-ftp-site-routing-$(date +%Y%m%d-%H%M%S)"
REPORT=/root/hyper-host-v68-dual-ftp-site-routing-report.txt
...
ROLLBACK_NEEDED=1
REDIRECT_EXISTED=0
TEST_USER="hhv68$(date +%s)"
TEST_PASS="V68$(openssl rand -hex 16)"
TEST_DIR="$(mktemp -d /tmp/hhv68-dual.XXXXXX)"
...
restore(){
  ...
  printf '[%b%s%b] Ошибка установки, возвращаю предыдущие FTP и Nginx-конфиги.\n' "$CYAN" "$PROJECT" "$RESET" >&2
...
printf 'HYPER-HOST v68 LAN transfer\n%.0s' {1..4096} > "$LAN_SRC"
printf 'HYPER-HOST v68 WAN transfer\n%.0s' {1..4096} > "$WAN_SRC"
...
cls -1
put $LAN_SRC -o v68-lan-test.bin
get v68-lan-test.bin -o $LAN_DST
rm v68-lan-test.bin
bye
LFTP
then
...
set net:max-retries 1
cls -1
put $WAN_SRC -o v68-wan-test.bin
get v68-wan-test.bin -o $WAN_DST
rm v68-wan-test.bin
bye
LFTP
then
...
nginx -t >/dev/null || fail 'Nginx-конфиг не прошёл проверку после исправления маршрутизации'
...
STUB_HTTP="$(curl -kfsSL --max-time 10 --resolve "$TEST_SITE:80:127.0.0.1" --resolve "$TEST_SITE:443:127.0.0.1" "http://$TEST_SITE/" 2>/dev/null || true)"
STUB_HTTPS="$(curl -kfsSL --max-time 10 --resolve "$TEST_SITE:443:127.0.0.1" "https://$TEST_SITE/" 2>/dev/null || true)"
grep -Fq "$TEST_SITE работает" <<<"$STUB_HTTP" || fail 'Новый сайт по HTTP не отдаёт собственную заглушку'
grep -Fq "$TEST_SITE работает" <<<"$STUB_HTTPS" || fail 'Новый сайт по HTTPS не отдаёт собственную заглушку'
SITE_MARKER="HYPER-HOST-v68-user-files-$(date +%s)-$$"
...
USER_HTTP="$(curl -kfsSL --max-time 10 --resolve "$TEST_SITE:80:127.0.0.1" --resolve "$TEST_SITE:443:127.0.0.1" "http://$TEST_SITE/" 2>/dev/null || true)"
USER_HTTPS="$(curl -kfsSL --max-time 10 --resolve "$TEST_SITE:443:127.0.0.1" "https://$TEST_SITE/" 2>/dev/null || true)"
grep -Fq "$SITE_MARKER" <<<"$USER_HTTP" || fail 'После замены index.html сайт по HTTP не показывает загруженные файлы'
grep -Fq "$SITE_MARKER" <<<"$USER_HTTPS" || fail 'После замены index.html сайт по HTTPS не показывает загруженные файлы'
UNKNOWN_HOST="v68-unknown-$(date +%s).test"
UNKNOWN_BODY="$(curl -kfsSL --max-time 10 --resolve "$UNKNOWN_HOST:80:127.0.0.1" "http://$UNKNOWN_HOST/" 2>/dev/null || true)"
grep -Fq 'Домен не настроен' <<<"$UNKNOWN_BODY" || fail 'Неизвестный домен всё ещё попадает не в neutral default vhost'
...
if [[ -d "$TARGET_ROOT" && -f "$TARGET_CONF" ]]; then
  TARGET_TOKEN="HYPER-HOST-v68-target-$TARGET_DOMAIN-$(date +%s)-$$"
  ...
  TARGET_HTTP="$(curl -kfsSL --max-time 12 --resolve "$TARGET_DOMAIN:80:127.0.0.1" --resolve "$TARGET_DOMAIN:443:127.0.0.1" "http://$TARGET_DOMAIN/$TARGET_PROBE" 2>/dev/null || true)"
  TARGET_HTTPS="$(curl -kfsSL --max-time 12 --resolve "$TARGET_DOMAIN:443:127.0.0.1" "https://$TARGET_DOMAIN/$TARGET_PROBE" 2>/dev/null || true)"
  [[ "$TARGET_HTTP" == "$TARGET_TOKEN" ]] || fail "$TARGET_DOMAIN по HTTP открывает не свой public_html"
  [[ "$TARGET_HTTPS" == "$TARGET_TOKEN" ]] || fail "$TARGET_DOMAIN по HTTPS открывает не свой public_html"
...
{
  echo 'HYPER-HOST v68 dual FTP + Nginx site routing'
  ...
  echo 'Nginx default vhost: neutral, panel is not exposed on unknown domains'
  echo 'New-site HTTP/HTTPS stub: passed'
...
printf '\n============================================================\n'
printf ' %b%s%b — локальный/публичный FTPS и сайты готовы\n' "$CYAN" "$PROJECT" "$RESET"
printf '============================================================\n'
printf ' LAN:                %s:21\n' "$LAN_IP"
