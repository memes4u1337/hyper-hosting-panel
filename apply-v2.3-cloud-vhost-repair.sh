...
# HYPER CLOUD v105 — vhost repair + standalone multi-user cloud.
# Usage: sudo ./apply-v2.3-cloud-vhost-repair.sh [/root/hyper-hosting-panel]
...
BACKUP="/opt/hyper-host/backups/cloud-v105-${STAMP}"
...
log "Резервирую $CLOUD_DOMAIN за отдельным Cloud-приложением"
MANAGED_DIR="/etc/nginx/hyper-host-managed"
CLOUD_HELPER_SRC="$SRC/scripts/hyper-cloud-nginx-fragment.sh"
CLOUD_HELPER_DST="/opt/hyper-host/bin/hyper-cloud-nginx-fragment.sh"
...
mkdir -p /opt/hyper-host/bin "$MANAGED_DIR"

if [[ -d "/var/www/hyper-host-sites/$CLOUD_DOMAIN" ]]; then
  touch "/var/www/hyper-host-sites/$CLOUD_DOMAIN/.hyper-host-disabled"
fi
if [[ -f "$PANEL_DB" ]]; then
  sqlite3 "$PANEL_DB" ".backup '$BACKUP/panel-db-before-cloud-v104.sqlite'" >/dev/null 2>&1 || true
  sqlite3 "$PANEL_DB" "DELETE FROM sites WHERE lower(domain)=lower('$CLOUD_DOMAIN');" >/dev/null 2>&1 || true
fi

[[ -f "$CLOUD_HELPER_SRC" ]] || { echo "[ERROR] Нет $CLOUD_HELPER_SRC" >&2; exit 1; }
install -m 0755 "$CLOUD_HELPER_SRC" "$CLOUD_HELPER_DST"

# v105 recovery from failed v103/v104 installs: quarantine any legacy site vhost
# that still claims the Cloud hostname outside hyper-host-managed.
for dir in /etc/nginx/sites-enabled /etc/nginx/sites-available; do
  [[ -d "$dir" ]] || continue
  while IFS= read -r conf; do
    [[ -n "$conf" ]] || continue
    base="$(basename "$conf")"
    cp -aL "$conf" "$BACKUP/legacy-${base}" 2>/dev/null || true
    rm -f -- "$conf"
...
patch_reconcile(){
  local file="$1"
  [[ -f "$file" ]] || return 0
  cp -a "$file" "$BACKUP/$(basename "$file").before-v104" 2>/dev/null || true
  python3 - "$file" <<'PYRECON'
from pathlib import Path
import sys
p=Path(sys.argv[1])
s=p.read_text(encoding='utf-8',errors='ignore')
marker='# HYPER CLOUD v104 persistent vhost hook'
hook="""\n# HYPER CLOUD v104 persistent vhost hook\nif [[ -x /opt/hyper-host/bin/hyper-cloud-nginx-fragment.sh ]]; then\n  /opt/hyper-host/bin/hyper-cloud-nginx-fragment.sh\nfi\n"""
if marker not in s:
    anchor='--map /opt/hyper-host/data/v89-routing.tsv\n'
    if anchor in s:
        s=s.replace(anchor,anchor+hook,1)
    else:
        anchor2='if ! TEST_OUTPUT="$(nginx -t 2>&1)"; then'
        if anchor2 not in s:
            raise SystemExit('Не найдено место для Cloud hook в nginx reconcile')
        s=s.replace(anchor2,hook+'\n'+anchor2,1)
p.write_text(s,encoding='utf-8')
PYRECON
  bash -n "$file"
}

patch_reconcile "/usr/local/sbin/hyper-host-nginx-reconcile"
patch_reconcile "$SRC/scripts/nginx-reconcile-v89.sh"

patch_recover_python(){
  local file="$1"
  [[ -f "$file" ]] || return 0
  cp -a "$file" "$BACKUP/$(basename "$file").before-v104" 2>/dev/null || true
  python3 - "$file" "$CLOUD_DOMAIN" <<'PYRECOVER'
from pathlib import Path
import sys
p=Path(sys.argv[1]); domain=sys.argv[2]
s=p.read_text(encoding='utf-8',errors='ignore')
marker='# HYPER CLOUD v104 reserved domain'
if marker not in s:
    anchor='    db_sites = load_db_sites(db_path)\n'
    injected=f'    # HYPER CLOUD v104 reserved domain\n    canonical = [d for d in canonical if d != "{domain}"]\n'
    if anchor not in s:
        raise SystemExit('Не найден canonical anchor в nginx_recover_v89.py')
    s=s.replace(anchor,injected+anchor,1)
    old='    for old in managed.glob("*.conf"):\n        old.unlink(missing_ok=True)\n'
    new='    for old in managed.glob("*.conf"):\n        # HYPER CLOUD v104 keeps its standalone managed vhost across direct recover runs.\n        if old.name == "15-cloud-app.conf":\n            continue\n        old.unlink(missing_ok=True)\n'
    if old not in s:
        raise SystemExit('Не найден managed cleanup anchor в nginx_recover_v89.py')
    s=s.replace(old,new,1)
p.write_text(s,encoding='utf-8')
PYRECOVER
  python3 -m py_compile "$file"
}
patch_recover_python "/opt/hyper-host/nginx_recover_v89.py"
patch_recover_python "$SRC/scripts/nginx_recover_v89.py"

rm -f /etc/nginx/sites-enabled/hyper-host-cloud /etc/nginx/sites-available/hyper-host-cloud 2>/dev/null || true
rm -f "$MANAGED_DIR/20-site-$CLOUD_DOMAIN.conf" 2>/dev/null || true

log "Пересобираю HYPER-HOST nginx и вставляю Cloud vhost после reconcile"
if [[ -x /usr/local/sbin/hyper-host-nginx-reconcile ]]; then
  # v105: pass the already-detected socket explicitly to the helper used by the hook.
  export PHP_FPM_SOCK="$FPM_SOCK"
  /usr/local/sbin/hyper-host-nginx-reconcile
  # Belt-and-suspenders repair: even if an older recover script removed the file,
  # recreate the dedicated Cloud vhost once more before TLS checks.
  "$CLOUD_HELPER_DST"
  nginx -t
  systemctl reload nginx
else
  PHP_FPM_SOCK="$FPM_SOCK" "$CLOUD_HELPER_DST"
  nginx -t
  systemctl reload nginx
fi

log "Проверяю/выпускаю SSL без certbot --nginx"
find_cloud_cert(){
  local base cert key
  for base in /opt/hyper-host/letsencrypt/live /etc/letsencrypt/live; do
    cert="$base/$CLOUD_DOMAIN/fullchain.pem"; key="$base/$CLOUD_DOMAIN/privkey.pem"
    if [[ -f "$cert" && -f "$key" ]] && openssl x509 -in "$cert" -noout -checkhost "$CLOUD_DOMAIN" >/dev/null 2>&1; then
      printf '%s\n' "$cert"; return 0
    fi
  done
  return 1
}
if ! find_cloud_cert >/dev/null 2>&1; then
  mkdir -p /opt/hyper-host/acme-webroot/.well-known/acme-challenge
  if getent ahostsv4 "$CLOUD_DOMAIN" >/dev/null 2>&1; then
    certbot certonly --webroot -w /opt/hyper-host/acme-webroot \
      -d "$CLOUD_DOMAIN" --non-interactive --agree-tos --register-unsafely-without-email \
      --keep-until-expiring || warn "Не удалось выпустить SSL. Cloud останется доступен по HTTP до повторного запуска."
  else
    warn "DNS $CLOUD_DOMAIN пока не разрешается; SSL пропущен."
  fi
fi
...
patch_panel(){
  local file="$1"
  [[ -f "$file" ]] || return 0
  python3 - "$file" "$CLOUD_DOMAIN" <<'PYPANEL'
from pathlib import Path
import re,sys
p=Path(sys.argv[1]); domain=sys.argv[2]; s=p.read_text()
s=re.sub(r"function hh_app_version\(\): string \{ return '[^']+'; \}", "function hh_app_version(): string { return '1.9-v105-cloud'; }", s)
...
log "Финальная проверка: домен обязан отдавать именно HYPER CLOUD, а не public_html"
...
PROTO=http
PORT=80
if find_cloud_cert >/dev/null 2>&1; then PROTO=https; PORT=443; fi
ROUTE="$(curl -ksS --max-time 10 --resolve "$CLOUD_DOMAIN:$PORT:127.0.0.1" "$PROTO://$CLOUD_DOMAIN/__hyper_cloud_route__" || true)"
if [[ "$ROUTE" != "HYPER_CLOUD_V105" ]]; then
  echo "[ERROR] Домен всё ещё попал не в Cloud vhost. Получено: ${ROUTE:0:200}" >&2
  echo "[DEBUG] Конфиги с server_name $CLOUD_DOMAIN:" >&2
  grep -RIlE "server_name[[:space:]].*${CLOUD_DOMAIN//./\\.}" /etc/nginx/hyper-host-managed /etc/nginx/sites-enabled /etc/nginx/sites-available 2>/dev/null | sed 's/^/ - /' >&2 || true
  exit 1
fi
LOGIN_HTML="$(curl -ksS --max-time 10 --resolve "$CLOUD_DOMAIN:$PORT:127.0.0.1" "$PROTO://$CLOUD_DOMAIN/" || true)"
if ! grep -Fq 'HYPER CLOUD' <<<"$LOGIN_HTML" || ! grep -Eq 'Вход в облако|Создать аккаунт|Мой диск' <<<"$LOGIN_HTML"; then
  echo "[ERROR] Route marker Cloud верный, но приложение не отдало интерфейс Cloud." >&2
  exit 1
fi
printf '[HYPER CLOUD] DOMAIN_OK: %s://%s/ -> HYPER CLOUD v105\n' "$PROTO" "$CLOUD_DOMAIN"

PUBLIC_IP="$(curl -4 -fsS --max-time 5 https://api.ipify.org 2>/dev/null || true)"
printf '\n\033[1;32m============================================================\033[0m\n'
printf '\033[1;32m HYPER CLOUD v105 установлен\033[0m\n'
printf ' URL: https://%s/\n' "$CLOUD_DOMAIN"
