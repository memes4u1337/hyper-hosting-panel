...
BACKUP="/root/hyper-cs16-fastdl-v12-${STAMP}"
...
echo "================================================================"
echo " HYPER-HOST CS 1.6 FASTDL IP FINAL v12"
echo " Server:     #$SID"
...
# Replace fastdl_sync completely. Older local patches added an HTTP health check
# inside this function. That creates a circular dependency: sync cannot finish
# until nginx is fixed, while nginx was fixed only after sync. Keep sync limited
# to filesystem mirroring + cfg/state; HTTP validation is done later by this patch.
fs=text.find('def fastdl_sync(sid:int, configure:bool=True):')
fe=text.find('\ndef fastdl_status',fs)
if fs<0 or fe<0: raise SystemExit('cannot find fastdl_sync()')
clean_sync='''def fastdl_sync(sid:int, configure:bool=True):
    """Mirror client-downloadable assets. No HTTP/network dependency here."""
...
'''
text=text[:fs]+clean_sync+text[fe:]
marker='# HYPER-HOST FASTDL IP FINAL v12'
...
for p in (runtime,state):
    ...
    t=p.with_name('.'+p.name+'.v12.tmp'); t.write_text(json.dumps(d,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); t.replace(p)
    print('updated:',p)
PY

echo "[5/11] Rebuild FastDL mirror through clean controller (no HTTP dependency)..."
...
for p in loaded:
    ...
    # HYPER-HOST FASTDL IP FINAL v12
    location ^~ /fastdl/ {{
    ...
    location / {{ return 301 https://{domain}$request_uri; }}
            ...
            new=re.sub(r'(?ms)\n\s*# HYPER-HOST FASTDL IP FINAL v12 BEGIN.*?# HYPER-HOST FASTDL IP FINAL v12 END\s*\n?','\n',new)
            ins='''
    # HYPER-HOST FASTDL IP FINAL v12 BEGIN
    location ^~ /fastdl/ {
        alias /srv/hyper-cs16/fastdl/;
        autoindex off;
        sendfile on;
    ...
    # HYPER-HOST FASTDL IP FINAL v12 END
'''
            x=new.find('{')+1; new=new[:x]+ins+new[x:]; reps.append((a,b,new))
    if reps:
        safe=p.as_posix().strip('/').replace('/','__'); before=backup/(safe+'.before-v12')
...
echo
echo "================================================================"
echo " [SUCCESS] HYPER-HOST FASTDL IP FINAL v12"
echo "================================================================"
